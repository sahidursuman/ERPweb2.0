/*TMODJS:{"debug":true,"version":29,"md5":"5f9d136dc2b6034a4e0bb8d27bdb351d"}*/
    ...
    return require("../../../template")("arrange/arrangeTourist/view/chooseTravelLine", function($data, $filename) {
        try {
            ...
            $line = 16, $each(tripPlanList, function(tripPlanList) {
                ...
                $out += "</td> <td>", $line = 27, $out += $escape(tripPlanList.touristAdultCount), 
                $out += "大", $line = 27, $out += $escape(tripPlanList.touristChildCount), $out += "小</td> <td>", 
                ...
                $line = 35;
            }), $out += ' </tbody> </table> <form class="form-horizontal" role="form" onsubmit="return false"> <div class="form-group" style="text-align: center;margin:0px;"> <button class="btn btn-block btn-primary btn-chooseMergeTripPlan guideSubmit"> <i class="ace-icon fa fa-check"></i> 选择计划 </button> </div> </form> <div style="padding:20px"></div> </div> </div> ', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
