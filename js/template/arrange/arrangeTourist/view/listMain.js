/*TMODJS:{"debug":true,"version":55,"md5":"67225748426e3797b103fc91063f9c18"}*/
define(function(require) {
    return require("../../../template")("arrange/arrangeTourist/view/listMain", function($data, $filename) {
        try {
            ...
            }), $out += ' </select> </div> <label class="col-sm-1 control-label no-padding-right">出游日期：</label> <div class="col-sm-2"> <input name="startTime" value="" type="text" class="date-picker col-xs-12 guideInputList"/> </div> <button class=" btn-sm btn-arrangeTourist-search search-btn" style="margin-left: -22px"> <i class="ace-icon fa fa-search"></i> 搜索 </button> </div> </form> </div> <div class="row"> <div class="col-xs-12 arrangeTouristList"> </div> </div> <div class="col-xs-12 arrangeTouristMergeList"> <div> <button class="btn btn-sm btn-success btn-start-touristGroup-merge"> <i class="ace-icon fa fa-user-plus"></i> 开始并团 </button> </div> <div class="space-10"></div> <div class="list"> </div> </div> </div> </div>', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
