/*TMODJS:{"debug":true,"version":66,"md5":"a7c6aac5166cad256b2fa769f33ac20b"}*/
define(function(require) {
    return require("../../../template")("arrange/arrangeGroupTransfer/view/listTransfer", function($data, $filename) {
        try {
            ...
            return $out += '<table class="table table-striped table-bordered table-hover globalAdd T-showHighLight"> <colgroup> <col style="width: 45px;"> <col style="width: 300px;"> <col> <col> <col> <col> <col> <col> <col> <col> <col> <col> </colgroup> <thead> <tr class="bg-blur T-tr-fixed"> <th>选择</th> <th>收客信息</th> <th>线路类型</th> <th>天数</th> <th>出游日期</th> <th>联系人</th> <th>联系电话</th> <th>人数</th> <th>客源地</th> <th>年龄</th> <th>备注</th> <th>操作</th> </tr> </thead> <tbody class="T-arrageTransfer-list"> ', 
            $line = 33, $each(touristGroupList, function(group) {
                $out += " <tr data-value=", $line = 34, $out += $escape(group.id), $out += " data-entity-adultCount = ", 
                $line = 34, $out += $escape(group.adultCount), $out += " data-entity-childCount = ", 
                $line = 34, $out += $escape(group.childCount), $out += '> <td><label class="pos-rel"> <input type="checkbox" class="ace T-transferCheckBox T-cheked"> <span class="lbl"></span> </label></td> <td class="h-touristGroupInfo"> <p> ', 
                $line = 38, group.orderNumber && ($out += '<span class="h-orderNumber">收客单号：', $line = 38, 
                $out += $escape(group.orderNumber), $out += "</span>", $line = 38), $out += ' <span class="h-agencyName">', 
                $line = 39, null != group.lineProduct && ($out += " ", $line = 40, 0 == group.lineProduct.customerType ? ($out += "散客 ", 
                $line = 41) : 1 == group.lineProduct.customerType ? ($out += "团体 ", $line = 42) : 2 == group.lineProduct.customerType && ($out += "转客 ", 
                $line = 43), $out += " ", $line = 44), $out += "</span> </p> ", $line = 46, group.lineProduct && ($out += ' <p class="T-ctrl-tip h-lineName" title="', 
                $line = 47, $out += $escape(group.lineProduct.name), $out += '"><span>', $line = 47, 
                $out += $escape(group.lineProduct.name), $out += "</span></p> ", $line = 48), $out += " <p>", 
                $line = 49, null != group.partnerAgency && ($line = 49, $out += $escape(group.partnerAgency.travelAgencyName), 
                $line = 49), $out += "</p> </td> <td>", $line = 51, null != group.lineProduct && ($line = 51, 
                $out += $escape(group.lineProduct.type), $line = 51), $out += '</td> <td class="F-float F-count">', 
                $line = 52, null != group.lineProduct && ($line = 52, $out += $escape(group.lineProduct.days), 
                $line = 52), $out += "</td> <td>", $line = 53, $out += $escape($helpers.dateFormat(group.startTime, "yyyy-MM-dd")), 
                $out += "</td> ", $line = 54, group.contactMember && ($out += " <td>", $line = 55, 
                $out += $escape(group.contactMember.name), $out += "</td> <td>", $line = 56, $out += $escape(group.contactMember.mobileNumber), 
