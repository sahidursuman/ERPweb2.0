/*TMODJS:{"debug":true,"version":44,"md5":"3580bdad349d604698df04ec92002526"}*/
    ...
    return require("../../../template")("arrange/arrangeTravels/view/viewTravelShowList", function($data, $filename) {
        try {
            ...
            $line = 15, $each(noteList, function(rs) {
                ...
                $line = 17, $out += $escape(rs.id), $out += '"> <img class="cursor T-action T-view-group" src="', 
                $line = 18, $out += $escape(rs.coverImage ? imgUrl + rs.coverImage : "/images/loading-error.png"), 
                $out += '" alt="', $line = 18, $out += $escape(rs.title), $out += '"> <div class="caption"> <h3 class="cursor T-action T-view-group">', 
                $line = 20, $out += $escape(rs.title), $out += '</h3> <div class="caption-info"> <div class="caption-user pull-left cursor T-action T-view-user"> <div class="caption-user-header pull-left', 
                $line = 23, rs.guideInfo.photo || ($out += " default-img", $line = 23), $out += '"> ', 
                $line = 24, rs.guideInfo.photo && ($out += '<img src="', $line = 24, $out += $escape(imgUrl + rs.guideInfo.photo), 
                $out += '">', $line = 24), $out += ' </div> <div class="caption-user-info pull-left"> <p>', 
                $line = 27, $out += $escape(rs.guideInfo.name), $out += '</p> <p class="starLevel">', 
                $line = 28, $out += $string($helpers.getScoreStar(rs.guideInfo.guideSatisfaction)), 
                ...
                $line = 33, $out += $escape(rs.likeCount), $out += '</label> <label class="control-label mar-l-10 cursor T-action T-view-comment"><i class="fa fa-comment"></i>', 
                $line = 34, $out += $escape(rs.commentCount), $out += '</label> </p> <p class="text-right caption-date">', 
                $line = 36, $out += $escape(rs.updateTime), $out += "</p> </div> </div> </div> </div> </div> ", 
                $line = 42;
            }), $out += ' </div> <div class="row pageMode"> <div class="col-xs-4"> <small>共计 ', 
            $line = 47, $out += $escape(recordSize), $out += ' 条记录</small> </div> <div class="col-xs-8"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div> </div>', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
