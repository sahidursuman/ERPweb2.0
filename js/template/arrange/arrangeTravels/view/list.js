/*TMODJS:{"debug":true,"version":16,"md5":"da81dd2d66b0bdf4aecbe7e0e5b14373"}*/
define(function(require) {
    return require("../../../template")("arrange/arrangeTravels/view/list", function($data, $filename) {
        try {
            var $utils = this, $helpers = $utils.$helpers, $line = 0, $escape = $utils.$escape, lineProductName = $data.lineProductName, $string = $utils.$string, customerType = $data.customerType, $each = $utils.$each, lineProductList = $data.lineProductList, totalCount = ($data.rs, 
            ...
            $line = 39, $each(lineProductList, function(rs) {
                ...
                $out += $escape(rs.sumTouristAdultCount + rs.sumTouristChildCount), $out += "</td> <td>", 
                $line = 47, $out += $escape(rs.sumLikeCount), $out += "</td> <td>", $line = 48, 
                $out += $escape(rs.sumCommentCount), $out += "</td> <td>", $line = 49, $out += $escape(rs.sumShareCount), 
                $out += "</td> <td>", $line = 50, $out += $escape(rs.sumViewCount), $out += "</td> <td>", 
                $line = 51, $out += $escape(rs.createTime), $out += '</td> <td> <a class="cursor T-action T-view" title="查看">查看</a> <label style="padding-left:10px;">|</label> <a class="cursor T-action T-share" title="分享">分享</a> </td> </tr> ', 
                $line = 58;
            }), $out += ' </tbody> </table> </div> <div class="row pageMode"> <div class="col-xs-4"> <small>共计 ', 
            $line = 64, $out += $escape(totalCount), $out += ' 条记录</small> </div> <div class="col-xs-8"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div> </div> ', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
