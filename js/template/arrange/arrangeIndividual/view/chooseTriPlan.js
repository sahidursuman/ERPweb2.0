/*TMODJS:{"debug":true,"version":4,"md5":"a0ec0c4450e02f3f32fdc144df54110a"}*/
    ...
    return require("../../../template")("arrange/arrangeIndividual/view/chooseTriPlan", function($data, $filename) {
        try {
            ...
            $line = 23, $each(tripPlanList, function(tripPlanList) {
                ...
                $line = 27, null != tripPlanList.lineProduct && ($line = 27, $out += $escape(tripPlanList.lineProduct.name), 
                $line = 27), $out += "</td> <td>", $line = 28, $out += $escape($helpers.dateFormat(tripPlanList.startTime, "yyyy-MM-dd")), 
                $out += "</td> <td>", $line = 29, $out += $escape($helpers.dateFormat(tripPlanList.endTime, "yyyy-MM-dd")), 
                $out += "</td> <td>", $line = 30, null != tripPlanList.guide && ($line = 30, $out += $escape(tripPlanList.guide.realname), 
                $line = 30), $out += '</td> <td class="F-float F-count"> 大人', $line = 32, $out += $escape(tripPlanList.touristAdultCount), 
                $out += "小孩", $line = 32, $out += $escape(tripPlanList.touristChildCount), $out += '</td> <td class="T-remainSeatCount F-float F-count">', 
                $line = 33, null != tripPlanList.bus && ($line = 33, $out += $escape(tripPlanList.bus.seatCount - tripPlanList.touristAdultCount - tripPlanList.touristChildCount), 
                $line = 33), $out += "</td> <td> ", $line = 35, 0 == tripPlanList.status ? ($out += "未发团 ", 
                $line = 36) : ($out += "已发团 ", $line = 37), $out += ' </td> <td> <label class="pos-rel"> <input type="radio" class="ace ridioCheck" name="form-field-radio"> <span class="lbl"></span> </label> </td> </tr>', 
                $line = 45;
