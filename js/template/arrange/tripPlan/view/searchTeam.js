/*TMODJS:{"debug":true,"version":10,"md5":"fd48ffd581392726a6b2e2af124f1468"}*/
    ...
    return require("../../../template")("arrange/tripPlan/view/searchTeam", function($data, $filename) {
        try {
            ...
            $line = 21, $each(tourisGroupList, function(rs) {
                $out += ' <tr data-id="', $line = 22, $out += $escape(rs.id), $out += '"> <td name="partnerAgencyName">', 
                $line = 23, $out += $escape(rs.fromPartnerAgencyName), $out += "</td> <td>", $line = 24, 
                $out += $escape(rs.lineProductName), $out += "</td> <td>", $line = 25, $out += $escape(rs.customerType), 
                $out += "</td> <td>", $line = 26, $out += $escape(rs.startTime), $out += "</td> <td>", 
                $line = 27, $out += $escape(rs.quoteNumber), $out += "</td> <td>", $line = 28, $out += $escape(rs.contactMemberName), 
                $out += "</td> <td>", $line = 29, $out += $escape(rs.adultCount), $out += "大", $line = 29, 
                0 != rs.childCount && ($line = 29, $out += $escape(rs.childCount), $out += "小", 
                $line = 29), $out += "</td> <td>", $line = 30, 1 == rs.buyInsurance ? ($out += "是", 
                $line = 30) : ($out += "否", $line = 30), $out += "</td> <td>", $line = 31, $out += $escape(rs.otaOrderNumber), 
                $out += "</td> <td>", $line = 32, 1 == rs.memberType ? ($out += "外宾", $line = 32) : ($out += "内宾", 
                $line = 32), $out += "</td> <td>", $line = 33, $out += $escape(rs.welcomeBoard), 
                $out += "</td> <td>", $line = 34, $out += $escape(rs.outOPUserName), $out += "</td> <td>", 
                $line = 35, $out += $escape(rs.createTime), $out += '</td> <td> <label class="T-team-radio"> <input type="radio" class="ace" name="team-radio"> <span class="lbl"> </span> </label> </td> </tr> ', 
                $line = 43;
