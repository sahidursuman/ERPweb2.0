/*TMODJS:{"debug":true,"version":69,"md5":"b1b19c121fe27fd732155b341186de1e"}*/
        ...
        try {
            ...
            $line = 19, $each(data, function(rs) {
                ...
                $out += $escape(rs.condition), $out += "</td> ", $line = 24, $each(rs.busCompanyOfferList, function(offer, index) {
                    ...
                    $out += '</td> <td class="T-price">', $line = 56, "已同意" == offer.status ? ($line = 56, 
                    $out += $escape(offer.replyPrice), $line = 56) : ($out += "-", $line = 56), $out += "</td> <td>", 
                    $line = 57, $out += $escape(offer.remark), $out += "</td> <td>", $line = 58, -1 == offer.reserveMinutes ? ($out += " ", 
                    $line = 59, "已拒绝" == offer.status ? ($out += " - ", $line = 61) : ($out += " 一直保留 ", 
                    $line = 63), $out += " ", $line = 64) : 0 == offer.reserveMinutes ? ($out += " - ", 
                    $line = 66) : ($out += " ", $line = 67, $out += $escape(offer.reserveTime), $out += " ", 
                    $line = 68), $out += " </td> <td>", $line = 70, $out += $escape(offer.expiryTime), 
