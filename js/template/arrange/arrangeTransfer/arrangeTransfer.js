define(function(require, exports) {
	...
	var transfer = {
					...
					success:function(data){
						...
						$transferIn.find(".btn-transferIn-search").click(function(){
							transfer.searchInData = {
									...
									status : $transferIn.find("select[name=status]").val(),
			...
			//接受同行转入  
//			acceptTransferIn:function(){  
//				var statusVlue=$(this).parent().parent().parent().find("span").attr("data-status"),
//				id = $(this).attr("data-entity-id");
//				var dialogObj = $( "#confirm-dialog-message" );
//				if (statusVlue==1) {
//					showMessageDialog($( "#confirm-dialog-message" ),"你已确认接收！");
//					return ;
//				}else {
//				dialogObj.removeClass('hide').dialog({
//					modal: true,
...
//			
//									$.ajax({
//										url:""+APP_ROOT+"back/transfer.do?method=saveTourist&token="+$.cookie("token")+"&menuKey="+menuKey+"&operation=view",
//										type:"POST",
//										data:"tourGroupTransferId="+id + "&isDelete=1",
//										dataType:"json",
//										beforeSend:function(){
//											globalLoadingLayer = layer.open({
//												type:3
			...
			PayMoneyF:function(){
				...
				for(i=0;i<trList.length;i++){
					...
					var b =parseFloat(trList.eq(i).find(".price").val());
						...
						/*if(trList.eq(i).find("option:selected").val()==0){//增加
							a=a;
						}
						else if(trList.eq(i).find("option:selected").val()==1){//减少
							a=-a;
						}*/
						if(isNaN(a)){
							a = 0;
