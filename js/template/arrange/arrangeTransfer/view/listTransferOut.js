/*TMODJS:{"debug":true,"version":279,"md5":"914835f91180e949f24a1de9a7dff5e3"}*/
    ...
    return require("../../../template")("arrange/arrangeTransfer/view/listTransferOut", function($data, $filename) {
        try {
            ...
            $line = 17, $each(result, function(result) {
                ...
                $line = 56) : 3 == result.status ? ($out += '<span style="">未使用</span> ', $line = 57) : 4 == result.status && ($out += '<span style="color: red">已退回</span> ', 
                $line = 58), $out += ' </td> <td> <div class="hidden-sm hidden-xs btn-group"> <a data-value="', 
                $line = 62, $out += $escape(result.id), $out += '" class="cursor T-transfer-view T-action"> 查看 </a> <a data-value="', 
                $line = 65, $out += $escape(result.id), $out += '" class="cursor T-transfer-edit T-action R-right" data-right="1180003"> <label class="padding-right20">|</label> 编辑 </a> <a data-value="', 
                $line = 70, $out += $escape(result.id), $out += '" class="cursor ', $line = 70, 
                1 != result.status && 2 != result.status && ($out += "T-transfer-delete", $line = 70), 
                $out += ' T-action R-right" ', $line = 70, (1 == result.status || 2 == result.status) && ($out += 'style="color:#bbb;"', 
                ...
                1 != result.status && 2 != result.status && ($out += "T-transfer-confirm", $line = 75), 
                $out += ' T-action R-right" ', $line = 75, (1 == result.status || 2 == result.status) && ($out += 'style="color:#bbb;"', 
                $line = 75), $out += ' data-right="1180004"> <label class="padding-right20">|</label> 确认 </a> <a data-value="', 
                $line = 80, $out += $escape(result.id), $out += '" class="cursor ', $line = 80, 
                4 == result.status && ($out += "T-returnTransfer-confirm", $line = 80), $out += ' T-action R-right" data-right="1180004" ', 
                $line = 80, 4 != result.status && ($out += 'style="color:#bbb;"', $line = 80), $out += '> <label class="padding-right20">|</label> 确认退回 </a> </div> </td> </tr> ', 
                $line = 89), $out += " ", $line = 90;
