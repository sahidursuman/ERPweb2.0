/*TMODJS:{"debug":true,"version":250,"md5":"76b3153179c36134db3b67288f39590f"}*/
    ...
    return require("../../../template")("arrange/arrangeTransfer/view/listTransferIn", function($data, $filename) {
        try {
            ...
            $line = 18, $each(result, function(result) {
                ...
                $line = 44) : 1 == result.status ? ($out += '<span style="color: green;" data-status="1">已接收</span> ', 
                $line = 45) : 2 == result.status ? ($out += '<span style="color: red;" data-status="2">已拒收</span> ', 
                $line = 46) : 4 == result.status ? ($out += '<span style="color: green;" data-status="4">申请退回</span> ', 
                $line = 47) : 5 == result.status && ($out += '<span style="color: red">已退回</span> ', 
                $line = 48), $line = 48), $out += '</td> <td> <div class="hidden-sm hidden-xs btn-group"> <a data-value="', 
                $line = 52, $out += $escape(result.id), $out += '" class="cursor T-transferIn-view T-action"> 查看 </a> <a data-value="', 
                $line = 55, $out += $escape(result.id), $out += '" class="cursor ', $line = 55, 
                1 != result.status && 5 != result.status && ($out += "T-transferIn-confirm", $line = 55), 
                $out += ' R-right T-action" style="', $line = 55, (1 == result.status || 5 == result.status) && ($out += " color:#bbb; ", 
                $line = 55), $out += '" data-right="1180007"> <label class="padding-right20">|</label> 确认 </a> <a data-value="', 
                $line = 59, $out += $escape(result.id), $out += '" class="cursor ', $line = 59, 
                2 != result.status && 1 != result.status && 5 != result.status && ($out += "T-transferIn-refuse", 
                $line = 59), $out += ' R-right T-action" style="', $line = 59, (2 == result.status || 1 == result.status || 5 == result.status) && ($out += " color:#bbb; ", 
                $line = 59), $out += '" data-right="1180008"> <label class="padding-right20">|</label> 拒绝 </a> <a data-value="', 
                $line = 64, $out += $escape(result.id), $out += '" class="cursor ', $line = 64, 
                1 == result.status && ($out += "T-returnTransferIn-refuse", $line = 64), $out += ' R-right T-action" style="', 
                $line = 64, 1 != result.status && ($out += " color:#bbb; ", $line = 64), $out += '" data-right="1180008"> <label class="padding-right20">|</label> 申请退回 </a> </div> </td> </tr> ', 
                $line = 72;
            }), $out += ' </tbody> </table> <div class="row pageMode"> <div class="col-xs-5"> <small>共计 ', 
            $line = 77, $out += $escape(searchParam.totalCount), $out += '条记录</small> </div> <div class="col-xs-7"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div> ', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
