/*TMODJS:{"debug":true,"version":266,"md5":"d7ef988e46375b89b9a0f886bce99c4e"}*/
    ...
    return require("../../../template")("arrange/arrangeTransfer/view/listTransferIn", function($data, $filename) {
        try {
            ...
            $line = 18, $each(result, function(result) {
                ...
                $line = 46) : 4 == result.status && ($out += '<span style="color: red;" data-status="4">已退回</span> ', 
                $line = 47), $line = 47), $out += '</td> <td> <div class="hidden-sm hidden-xs btn-group"> <a data-value="', 
                $line = 51, $out += $escape(result.id), $out += '" class="cursor T-transferIn-view T-action"> 查看 </a> <a data-value="', 
                $line = 54, $out += $escape(result.id), $out += '" class="cursor ', $line = 54, 
                1 != result.status && 5 != result.status && ($out += "T-transferIn-confirm", $line = 54), 
                $out += ' R-right T-action" style="', $line = 54, (1 == result.status || 5 == result.status) && ($out += " color:#bbb; ", 
                $line = 54), $out += '" data-right="1180007"> <label class="padding-right20">|</label> 确认 </a> <a data-value="', 
                ...
                $line = 58), $out += ' R-right T-action" style="', $line = 58, (2 == result.status || 1 == result.status || 5 == result.status) && ($out += " color:#bbb; ", 
                $line = 58), $out += '" data-right="1180008"> <label class="padding-right20">|</label> 拒绝 </a> <a data-value="', 
                $line = 63, $out += $escape(result.id), $out += '" class="cursor ', $line = 63, 
                1 == result.status && ($out += "T-returnTransferIn-refuse", $line = 63), $out += ' R-right T-action" style="', 
                $line = 63, 1 != result.status && ($out += " color:#bbb; ", $line = 63), $out += '" data-right="1180008"> <label class="padding-right20">|</label> 退回 </a> </div> </td> </tr> ', 
                $line = 71;
            }), $out += ' </tbody> </table> <div class="row pageMode"> <div class="col-xs-5"> <small>共计 ', 
            $line = 76, $out += $escape(searchParam.totalCount), $out += '条记录</small> </div> <div class="col-xs-7"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div> ', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
