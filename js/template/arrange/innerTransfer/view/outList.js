/*TMODJS:{"debug":true,"version":267,"md5":"c631751ce47806ff225099184e9e606a"}*/
define(function(require) {
    return require("../../../template")("arrange/innerTransfer/view/outList", function($data, $filename) {
        try {
            ...
            $data.$index, $utils.$escape), result = $data.result, searchParam = $data.searchParam, $out = "";
            ...
            $line = 15, $each(resultList, function(list) {
                ...
                $out += $escape(list.createTime), $out += "</td> <td>", $line = 24, 0 == list.status ? ($out += '<span style="color:#D2691E;">未确认</span>', 
                $line = 24) : 1 == list.status ? ($out += '<span style="color: green;">已确认</span> ', 
                $line = 25) : 2 == list.status ? ($out += '<span style="color: red;">已拒收</span> ', 
                $line = 26) : 4 == list.status ? ($out += '<span style="color: green;">申请退回</span> ', 
                $line = 27) : 5 == result.status && ($out += '<span style="color: red">已退回</span> ', 
                $line = 28), $out += '</td> <td> <div class="hidden-sm hidden-xs btn-group"> <a class="cursor ', 
                $line = 31, 1 != list.status && ($out += "T-TransferOut-edit", $line = 31), $out += ' R-right T-action" data-right="1340002" ', 
                $line = 31, 1 == list.status && ($out += 'style="color:#bbb"', $line = 31), $out += '> 编辑 <label style="padding-left:10px;">|</label> </a> <a class="cursor T-TransferOut-view T-action"> 查看 </a> <a class="cursor ', 
                $line = 38, 1 != list.status && 2 == list.status && ($out += "T-TransferOut-delete", 
                $line = 38), $out += ' R-right T-action" data-right="1340003" ', $line = 38, (1 == list.status || 2 == list.status) && ($out += 'style="color:#bbb"', 
                $line = 38), $out += '> <label class="padding-right20">|</label> 撤销 </a> <a class="cursor ', 
                $line = 43, 4 == list.status && ($out += "T-returnTransferOut-delete", $line = 43), 
                $out += ' R-right T-action" data-right="1340003" ', $line = 43, 4 != list.status && ($out += 'style="color:#bbb"', 
                $line = 43), $out += '> <label class="padding-right20">|</label> 确认退回 </a> </div> </td> </tr> ', 
                $line = 51;
            }), $out += ' </tbody> </table> <div class="row pageMode"> <div class="col-xs-6"> <small>共计&nbsp;', 
            $line = 56, $out += $escape(searchParam.totalCount), $out += '&nbsp;条记录</small> </div> <div class="col-xs-6"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div>', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
