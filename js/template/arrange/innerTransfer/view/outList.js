/*TMODJS:{"debug":true,"version":16,"md5":"4f551d120b6cc5ebd142d6405ea7efa9"}*/
define(function(require) {
    return require("../../../template")("arrange/innerTransfer/view/outList", function($data, $filename) {
        try {
            ...
            return $out += '<table class="table table-striped table-bordered table-hover T-showHighLight"> <thead> <tr class="bg-blur"> <th>收客单号</th> <th>接收部门</th> <th>出游日期</th> <th>游客</th> <th>联系电话</th> <th>人数</th> <th>应付款</th> <th>转客人</th> <th>转客时间</th> <th>对方状态</th> <th>操作</th> </tr> </thead> <tbody class="T-transferOut-list">', 
            $line = 17, $each(resultList, function(list) {
                ...
                $line = 19), $out += "</td> <td>", $line = 20, $out += $escape(list.toBusinessGroup.name), 
                $out += "</td> <td>", $line = 21, list.touristGroup && ($line = 21, $out += $escape($helpers.dateFormat(list.touristGroup.startTime, "yyyy-MM-dd")), 
                $line = 21), $out += "</td> <td>", $line = 22, null != list.touristGroup.contactMember && ($line = 22, 
                $out += $escape(list.touristGroup.contactMember.name), $line = 22), $out += "</td> <td>", 
                $line = 23, null != list.touristGroup.contactMember && ($line = 23, $out += $escape(list.touristGroup.contactMember.mobileNumber), 
                $line = 23), $out += '</td> <td><span class=" F-float F-count">', $line = 24, $out += $escape(list.touristGroup.adultCount), 
                $out += '</span>大<span class=" F-float F-money">', $line = 24, $out += $escape(list.touristGroup.childCount), 
                $out += '</span>小</td> <td><span class=" F-float F-money">', $line = 25, $out += $escape(list.transNeedPayMoney), 
                $out += "</span></td> <td>", $line = 26, $out += $escape(list.user.realName), $out += "</td> <td>", 
                $line = 27, $out += $escape(list.createTime), $out += "</td> <td> ", $line = 29, 
                0 == list.status ? ($out += '<span style="color:#D2691E;">未确认</span> ', $line = 30) : 1 == list.status ? ($out += '<span style="color: green;">已确认</span> ', 
                $line = 31) : 2 == list.status ? ($out += '<span style="color: red;">已拒收</span> ', 
                $line = 32) : 4 == list.status && ($out += '<span style="color: red;">已退回</span> ', 
                $line = 33), $out += '</td> <td> <div class="btn-group"> <a class="cursor ', $line = 36, 
                1 != list.status && 4 != list.status && 2 != list.status && ($out += "T-TransferOut-edit", 
                $line = 36), $out += ' R-right T-action" data-right="1340002" ', $line = 36, (1 == list.status || 2 == list.status || 4 == list.status) && ($out += 'style="color:#bbb"', 
                $line = 36), $out += '> 编辑 <label style="padding-left:10px;">|</label> </a> <a class="cursor T-TransferOut-view T-action"> 查看 </a> <a class="cursor ', 
                $line = 43, 1 != list.status && 4 != list.status && ($out += "T-TransferOut-delete", 
                $line = 43), $out += ' R-right T-action" data-right="1340003" ', $line = 43, (1 == list.status || 4 == list.status) && ($out += 'style="color:#bbb"', 
                $line = 43), $out += '> <label class="padding-right20">|</label> 撤销 </a> <a class="cursor ', 
                $line = 48, 4 == list.status && ($out += "T-returnTransferOut-delete", $line = 48), 
                $out += ' R-right T-action" data-right="1340003" ', $line = 48, 4 != list.status && ($out += 'style="color:#bbb"', 
                $line = 48), $out += '> <label class="padding-right20">|</label> 确认退回 </a> </div> </td> </tr> ', 
                $line = 56;
