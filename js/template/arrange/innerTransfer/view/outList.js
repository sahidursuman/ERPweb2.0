/*TMODJS:{"debug":true,"version":168,"md5":"a564625053887f843b8dac11d3efbda3"}*/
define(function(require) {
    return require("../../../template")("arrange/innerTransfer/view/outList", function($data, $filename) {
        try {
            ...
            return $out += '<table class="table table-striped table-bordered table-hover T-showHighLight"> <thead> <tr class="bg-blur"> <th>接收部门</th> <th>游客</th> <th>联系电话</th> <th>人数</th> <th>应付款</th> <th>已付款</th> <th>转客人</th> <th>转客时间</th> <th>对方状态</th> <th width="143">操作</th> </tr> </thead> <tbody class="T-transferOut-list">', 
            $line = 16, $each(resultList, function(list) {
                ...
                $out += $escape(list.touristGroup.contactMember.mobileNumber), $line = 20), $out += '</td> <td><span class=" F-float F-count">', 
                $line = 21, $out += $escape(list.touristGroup.adultCount), $out += '</span>大<span class=" F-float F-money">', 
                $line = 21, $out += $escape(list.touristGroup.childCount), $out += '</span>小</td> <td><span class=" F-float F-money">', 
                $line = 22, $out += $escape(list.transNeedPayMoney), $out += '</span></td> <td><span class=" F-float F-money">', 
                $line = 23, $out += $escape(list.transPayedMoney), $out += "</span></td> <td>", 
                $line = 24, $out += $escape(list.user.realName), $out += "</td> <td>", $line = 25, 
                $out += $escape(list.createTime), $out += "</td> <td>", $line = 26, 0 == list.status ? ($out += '<span style="color:#D2691E;">未确认</span>', 
                ...
                $line = 29, 1 != list.status && ($out += "T-TransferOut-edit", $line = 29), $out += ' R-right T-action" data-right="1340002" ', 
                $line = 29, 1 == list.status && ($out += 'style="color:#bbb"', $line = 29), $out += '> 编辑 <label style="padding-left:10px;">|</label> </a> <a class="cursor T-TransferOut-view T-action"> 查看 </a> <a class="cursor ', 
                $line = 36, 1 != list.status && ($out += "T-TransferOut-delete", $line = 36), $out += ' R-right T-action" data-right="1340003" ', 
                $line = 36, 1 == list.status && ($out += 'style="color:#bbb"', $line = 36), $out += '> <label class="padding-right20">|</label> 撤销 </a> </div> </td> </tr> ', 
                $line = 43;
