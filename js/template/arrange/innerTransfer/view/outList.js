/*TMODJS:{"debug":true,"version":295,"md5":"8174f05578fdc27fae4d3f58b1b001e6"}*/
    ...
    return require("../../../template")("arrange/innerTransfer/view/outList", function($data, $filename) {
        try {
            ...
            $line = 15, $each(resultList, function(list) {
                ...
                $line = 28) : 4 == list.status ? ($out += '<span style="color: green;">申请退回</span> ', 
                $line = 29) : 5 == list.status && ($out += '<span style="color: red">已退回</span> ', 
                $line = 30), $out += '</td> <td> <div class="hidden-sm hidden-xs btn-group"> <a class="cursor ', 
                $line = 33, 1 != list.status && ($out += "T-TransferOut-edit", $line = 33), $out += ' R-right T-action" data-right="1340002" ', 
                $line = 33, 1 == list.status && ($out += 'style="color:#bbb"', $line = 33), $out += '> 编辑 <label style="padding-left:10px;">|</label> </a> <a class="cursor T-TransferOut-view T-action"> 查看 </a> <a class="cursor ', 
                $line = 40, 1 != list.status && 2 == list.status && ($out += "T-TransferOut-delete", 
                $line = 40), $out += ' R-right T-action" data-right="1340003" ', $line = 40, (1 == list.status || 2 == list.status) && ($out += 'style="color:#bbb"', 
                $line = 40), $out += '> <label class="padding-right20">|</label> 撤销 </a> <a class="cursor ', 
                $line = 45, 4 == list.status && ($out += "T-returnTransferOut-delete", $line = 45), 
                $out += ' R-right T-action" data-right="1340003" ', $line = 45, 4 != list.status && ($out += 'style="color:#bbb"', 
                $line = 45), $out += '> <label class="padding-right20">|</label> 确认退回 </a> </div> </td> </tr> ', 
                $line = 53;
            }), $out += ' </tbody> </table> <div class="row pageMode"> <div class="col-xs-6"> <small>共计&nbsp;', 
            $line = 58, $out += $escape(searchParam.totalCount), $out += '&nbsp;条记录</small> </div> <div class="col-xs-6"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div>', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
