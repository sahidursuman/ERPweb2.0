/*TMODJS:{"debug":true,"version":90,"md5":"e5262a01aa55b33d8e7d7e302a5fc732"}*/
define(function(require) {
    return require("../../../template")("arrange/innerTransfer/view/inList", function($data, $filename) {
        try {
            ...
            return $out += '<table class="table table-striped table-bordered table-hover"> <thead> <tr class="bg-blur"> <th>线路产品</th> <th>天数</th> <th>出游日期</th> <th>游客</th> <th>人数</th> <th>部门</th> <th>联系人</th> <th>联系电话</th> <th>应收</th> <th>已收</th> <th>我方状态</th> <th width="140">操作</th> </tr> </thead> <tbody>', 
            $line = 18, $each(resultList, function(list) {
                ...
                $line = 33, 1 != list.status && ($out += "btn-TransferIn-save", $line = 33), $out += ' R-right" data-right="1340004" ', 
                $line = 33, 1 == list.status && ($out += 'style="color:#bbb"', $line = 33), $out += '> 确认 <label style="padding-left:10px;">|</label> </a> <a class="cursor btn-TransferIn-view"> 查看 </a> <a class="cursor ', 
                $line = 40, 1 != list.status && ($out += "btn-TransferIn-refuse", $line = 40), $out += ' R-right" data-right="1340005" ', 
            ...
            $line = 62, 0 == searchParam.totalPage ? ($out += " 0/0 ", $line = 64) : ($out += " ", 
            $line = 65, $out += $escape(searchParam.pageNo + 1), $out += "/", $line = 65, $out += $escape(searchParam.totalPage), 
            $out += " ", $line = 66), $out += ' </a> </li> <li class="paginate_button" aria-controls="dynamic-table" tabindex="0"><a class="next" href="javascript:void(0)">下一页</a></li> <li class="paginate_button" aria-controls="dynamic-table" tabindex="0"><a class="last" href="javascript:void(0)">尾页</a></li> </ul> </div> </div> </div>', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
