/*TMODJS:{"debug":true,"version":425,"md5":"f69c22c527a4492092d05b80a36a466a"}*/
    ...
    return require("../../../template")("arrange/orderManage/view/listHotel", function($data, $filename) {
        try {
            ...
            $line = 22, $each(hotelOrderList, function(hotOrderList) {
                ...
                $out += $escape(hotOrderList.hotel.remark), $line = 56), $out += '</td> <td rowspan="', 
                $line = 57, $out += $escape(hotOrderList.hotelOrderNeedRoomList.length), $out += '" style="vertical-align:middle"> ', 
                $line = 58, 0 == hotOrderList.statusStr ? ($out += " 已下单 ", $line = 60) : -1 == hotOrderList.statusStr ? ($out += " 已撤单 ", 
                $line = 62) : 1 == hotOrderList.statusStr ? ($out += " 已确认 ", $line = 64) : 2 == hotOrderList.statusStr ? ($out += " 以完成 ", 
                $line = 66) : 3 == hotOrderList.statusStr && ($out += " 进行中 ", $line = 68), $out += ' </td> <td rowspan="', 
                $line = 70, $out += $escape(hotOrderList.hotelOrderNeedRoomList.length), $out += '" style="vertical-align:middle"> <a class="cursor ', 
                $line = 71, (0 == hotOrderList.statusStr || 1 == hotOrderList.statusStr) && ($out += "T-listHotel-refuse", 
                $line = 71), $out += ' R-right T-action" data-right="1450002" style="', $line = 71, 
                (-1 == hotOrderList.statusStr || 2 == hotOrderList.statusStr || 3 == hotOrderList.statusStr) && ($out += "color:#bbb;", 
                $line = 71), $out += '"> 撤销 </a> </td> </tr> ', $line = 77, $each(hotOrderList.hotelOrderNeedRoomList, function(htOrderNList, index) {
                    $out += " ", $line = 78, index > 0 && ($out += " <tr> <td>", $line = 80, $out += $escape(htOrderNList.type), 
                    $out += "</td> <td>", $line = 81, $out += $escape(htOrderNList.needRoomCount), $out += "</td> <td>", 
                    $line = 82, $out += $escape(htOrderNList.price), $out += "</td> <td>", $line = 83, 
                    $out += $escape(htOrderNList.price), $out += "</td> </tr> ", $line = 85), $out += " ", 
                    $line = 86;
                }), $out += " ", $line = 87;
            }), $out += ' </tbody> </table>  <div class="row pageMode"> <div class="col-xs-4"> <small>共计', 
            $line = 93, $out += $escape(searchParam.totalCount), $out += '条记录</small> </div> <div style="width:320px; float:right;"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div>  </div> ', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
