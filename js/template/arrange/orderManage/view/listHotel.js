/*TMODJS:{"debug":true,"version":677,"md5":"8c253d6b9e5a132d95092b6c328409ed"}*/
    ...
    return require("../../../template")("arrange/orderManage/view/listHotel", function($data, $filename) {
        try {
            ...
            $line = 37, $each(hotelOrderList, function(hotOrderList) {
                ...
                $line = 76, -1 == hotOrderList.status ? ($out += " 已撤单 ", $line = 78) : 0 == hotOrderList.status ? ($out += " 已下单 ", 
                $line = 80) : 1 == hotOrderList.status ? ($out += " 已确认 ", $line = 82) : 2 == hotOrderList.status ? ($out += " 已完成 ", 
                $line = 84) : 3 == hotOrderList.status && ($out += " 进行中 ", $line = 86), $out += " </td> </tr> ", 
                $line = 90, $each(hotOrderList.hotelOrderNeedRoomList, function(htOrderNList, index) {
                    $out += " ", $line = 91, index > 0 && ($out += " <tr> <td>", $line = 93, $out += $escape(htOrderNList.type), 
                    $out += '</td> <td class="count">', $line = 94, $out += $escape(htOrderNList.needRoomCount), 
                    $out += '</td> <td class="price">', $line = 95, $out += $escape(htOrderNList.price), 
                    $out += '</td> <td class="totalMoney">', $line = 96, $out += $escape(htOrderNList.price), 
                    $out += "</td> </tr> ", $line = 98), $out += " ", $line = 99;
                }), $out += " ", $line = 100;
            }), $out += ' </tbody> </table>  <div class="row pageMode"> <div class="col-xs-6"> <small>共计 ', 
            $line = 107, $out += $escape(searchParam.totalCount), $out += ' 条记录</small> </div> <div class="col-xs-6"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div>  </div> ', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
