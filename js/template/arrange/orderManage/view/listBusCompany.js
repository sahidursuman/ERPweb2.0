/*TMODJS:{"debug":true,"version":681,"md5":"9de57a3023d2eaa08f22d6ed4e62f19d"}*/
    ...
    return require("../../../template")("arrange/orderManage/view/listBusCompany", function($data, $filename) {
        try {
            ...
            $line = 36, $each(busOrderList, function(busOrderList) {
                ...
                $out += '</td> <td class="F-float F-count">', $line = 42, busOrderList.busCompanyArrange && ($line = 42, 
                ...
                $line = 43), $out += '</td> <td class="F-float F-count">', $line = 44, $out += $escape(busOrderList.seatCount), 
                $out += '</td> <td class="F-float F-money">', $line = 45, $out += $escape(busOrderList.price), 
                $out += '</td> <td class="F-float F-money">', $line = 46, $out += $escape(busOrderList.seatCount * busOrderList.price), 
                $out += "</td> <td>", $line = 47, null != busOrderList.user && ($line = 47, $out += $escape(busOrderList.user.realName), 
                ...
                $line = 51, -1 == busOrderList.status ? ($out += " 已撤单 ", $line = 53) : 0 == busOrderList.status ? ($out += " 预定中 ", 
                $line = 55) : 1 == busOrderList.status && -1 == busOrderList.result ? ($out += " 已拒绝 ", 
                $line = 57) : 1 == busOrderList.status && 1 == busOrderList.result && ($out += " 已预订 ", 
                $line = 59), $out += " </td> </tr> ", $line = 62;
            }), $out += ' </tbody> </table>  <div class="row pageMode"> <div class="col-xs-6"> <small>共计 ', 
            $line = 70, $out += $escape(searchParam.totalCount), $out += ' 条记录</small> </div> <div class="col-xs-6"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div>  </div> ', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
