/*TMODJS:{"debug":true,"version":41,"md5":"6a1e35c1c1ab7535b9b7a4d675130be7"}*/
    ...
    return require("/js/template/template")("arrange/arrangeCenter/transferView/otherList", function($data, $filename) {
        try {
            ...
            $line = 18, $each(outOtherList, function(item) {
                ...
                $out += "</span><span>", $line = 23, $out += $escape(item.partnerAgencyName), $out += "</span><span>外链销售：", 
                ...
                $line = 28) : 2 == shuttleType && ($out += "送", $line = 28), $out += ' </td> <td rowspan="', 
                ...
                $out += $escape(item.size), $out += '"> ', $line = 35, $out += $escape(item.startTime), 
                ...
                $line = 88, $each(item.arrangeItems, function(arrange, level2) {
                    ...
                    $out += "</td>  <td> ", $line = 101, $each(arrange.arrangeResultList, function(list, level3) {
                        $out += ' <label class="inline-block ', $line = 102, level3 > 0 && ($out += " item-line ", 
                        $line = 102), $out += '">  ', $line = 104, $out += $escape(list.name), $out += "  ", 
                        $line = 106, $out += $escape(list.shift), $out += " </label> ", $line = 108;
