/*TMODJS:{"debug":true,"version":87,"md5":"73ad99b12c68f650c160dbf30f9e34a6"}*/
    ...
    return require("../../../template")("arrange/arrangeCenter/transferView/planCollection", function($data, $filename) {
        try {
            ...
            $line = 15, $each(collectionList, function(item) {
                $out += ' <tr data-entity-id="" > <td> <input type="hidden" name="id" value="', 
                $line = 18, $out += $escape(item.id), $out += '"> <input type="hidden" name="touristGroupId" value="', 
                $line = 19, $out += $escape(item.touristGroupId), $out += '"> <input type="hidden" name="collection" value="', 
                $line = 20, $out += $escape(item.collection), $out += '"> <p>收客单号：<span class="orderNumber">', 
                $line = 21, $out += $escape(item.orderNumber), $out += '</span></p> <p><span><span class="lineProductName">&lt;', 
                $line = 22, $out += $escape(item.lineProductName), $out += '&gt;</span></span></p> <p><span name="startTime">', 
                $line = 23, $out += $escape(item.startTime), $out += '</span> </td> <td> <p><span name="tgMemberName">', 
                $line = 26, $out += $escape(item.tgMemberName), $out += '</span></p> <p> <span name="adultCount">', 
                $line = 28, $out += $escape(item.adultCount), $out += '</span>大 <span name="childCount">', 
                $line = 29, $out += $escape(item.childCount), $out += '</span>小 </p> <p><span name="tgMemberMobileNumber ">', 
                $line = 31, $out += $escape(item.tgMemberMobileNumber), $out += "</span></p> </td> <td>", 
                $line = 33, $out += $escape(item.partnerAgencyName), $out += "</td> <td>", $line = 34, 
                $out += $escape(item.outOPUser), $out += '</td> <td><input type="text" name="collection" value="', 
                $line = 35, $out += $escape(item.collection), $out += '"></td> <td> <label class="pos-rel mar-r-10"> <input type="radio" ', 
                $line = 38, 0 == item.collectionType && ($out += ' checked="checked" value="0" ', 
                $line = 38), $out += ' name="collectionType" class="ace ridioCheck T-assign-check" class="T-assign-check"> <span class="lbl"></span> 代收团款 </label> <label class="pos-rel"> <input type="radio" ', 
                $line = 42, 1 == item.collectionType && ($out += ' checked="checked" value="1" ', 
                $line = 42), $out += ' name="collectionType" class="ace ridioCheck T-assign-check" class="T-assign-check"> <span class="lbl"></span> 代收增项 </label> </td> </tr> ', 
                $line = 46;
