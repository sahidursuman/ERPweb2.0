/*TMODJS:{"debug":true,"version":50,"md5":"b2fe91357051115682e7a6aa66ac8919"}*/
define(function(require) {
    return require("../../../template")("arrange/arrangeCenter/transferView/viewHotel", function($data, $filename) {
        try {
            ...
            $data.$index, $utils.$escape), outHotelList = ($data.itme, $data.index, $data.outHotelList), $out = ($data.view, 
            "");
            ...
            $line = 3, $each(outRemarkList, function(viewmian) {
                ...
                $out += $escape(viewmian.arriveTime), $out += '</label> --> <label class="control-label mar-r-20" data-contact="', 
                $line = 12, $out += $escape($helpers.stringify(viewmian.contactMemberList)), $out += '">游客小组联系人： ', 
                $line = 13, $each(viewmian.contactMemberList, function(itme, index) {
                    $out += " ", $line = 14, 0 == index ? ($out += " <span>", $line = 15, $out += $escape(itme.contactMemberName), 
                    $out += '</span> <span class="mar-r-10">(', $line = 16, $out += $escape(itme.contactMemberPhoneNumber), 
                    $out += ")</span> ", $line = 17) : index > 0 && ($out += ' <a class="T-view-contact">查看更多</a> ', 
                    $line = 19), $out += " ", $line = 20;
                }), $out += ' </label> <label class="control-label mar-r-20">线路名称：', $line = 22, 
                $out += $escape(viewmian.lineProductName), $out += '</label> <label class="control-label">外联销售：', 
                $line = 23, $out += $escape(viewmian.outOPUserName), $out += '</label> </td> </tr> <tr> <td style="text-align: left;"> <label class="control-label mar-r-20">客户名：', 
                $line = 28, $out += $escape(viewmian.partnerAgencyName), $out += ' </label> <label class="control-label mar-r-20">要求：', 
                $line = 29, $out += $escape(viewmian.require), $out += '</label> <!-- <label class="control-label mar-r-20">班次：', 
                $line = 30, $out += $escape(viewmian.shift), $out += '</label> --> <label class="control-label mar-r-20">类型：', 
                $line = 31, 1 == viewmian.shuttleType ? ($out += "接团", $line = 31) : 2 == viewmian.shuttleType ? ($out += "送团", 
                $line = 31) : 3 == viewmian.shuttleType && ($out += "返程住宿 ", $line = 31), $out += '</label> <label class="control-label mar-r-20">出游日期：', 
                $line = 32, $out += $escape(viewmian.startTime), $out += '</label> <label class="control-label">收客单号：', 
                $line = 33, $out += $escape(viewmian.tgOrderNumber), $out += "</label> </td> </tr> </tbody> </table> ", 
                $line = 38;
            ...
            $line = 60, $each(outHotelList, function(view) {
                $out += ' <tr data-entity-id="', $line = 61, $out += $escape(view.outHotelId), $out += '"> <td> ', 
                $line = 63, $out += $escape(view.checkInTime), $out += " </td> <td> ", $line = 66, 
                $out += $escape(view.checkOutTime), $out += " </td> <td> ", $line = 69, $out += $escape($helpers.getHotelLevelDesc(view.hotelLevel)), 
                $out += " </td> <td> ", $line = 72, $out += $escape(view.hotelName), $out += " </td> <td> ", 
                $line = 75, $out += $escape(view.hotelManagerName), $out += " </td> <td> ", $line = 78, 
                $out += $escape(view.hotelMobileNumber), $out += " </td> <td> ", $line = 81, $out += $escape(view.hotelRoomType), 
                $out += " </td> <td> ", $line = 84, $out += $escape(view.price), $out += " </td> <td> ", 
                $line = 87, $out += $escape(view.memberCount), $out += " </td> <td> ", $line = 90, 
                $out += $escape(view.reduceMoney), $out += " </td> <td> ", $line = 93, $out += $escape(view.needPayMoney), 
                $out += " </td> <td> ", $line = 96, $out += $escape(view.prePayMoney), $out += ' </td> <td> <a class="cursor T-collection" data-type="hotel">', 
                $line = 99, $out += $escape(view.collection), $out += "</a> </td> <td>", $line = 101, 
                $out += $escape(view.remark), $out += "</td> </tr>", $line = 102;
