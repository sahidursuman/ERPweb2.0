/*TMODJS:{"debug":true,"version":197,"md5":"526c3c3e187e2e25681db538800ea469"}*/
    ...
    return require("../../../template")("arrange/arrangeCenter/transferView/busList", function($data, $filename) {
        try {
            ...
            $line = 17, $each(outRemarkArrangeList, function(bus) {
                ...
                $line = 18, $out += $escape(bus.shuttleType), $out += '" data-touristGroupId="', 
                $line = 18, $out += $escape(bus.touristGroupId), $out += '" ', $line = 18, bus.checked && ($out += ' class="success" ', 
                ...
                $line = 37), $out += '"> </td> <td> <p></p> <p> <span class="adultCount">', $line = 42, 
                $out += $escape(bus.adultCount), $out += '</span>大 <span class="childCount">', $line = 43, 
                $out += $escape(bus.childCount), $out += "</span>小 </p> ", $line = 45, $each(bus.contactMemberList, function(itme) {
                    $out += ' <p><span class="contactMemberName">', $line = 46, $out += $escape(itme.contactMemberName), 
                    $out += '</span> <span class="contactMemberPhoneNumber">(', $line = 47, $out += $escape(itme.contactMemberPhoneNumber), 
                    $out += ")</span></p> ", $line = 48;
                }), $out += ' </td> <td><span class="arriveTime">', $line = 50, $out += $escape(bus.arriveTime), 
                $out += '</span></td> <td><span class="shift">', $line = 51, $out += $escape(bus.shift), 
                $out += '</span></td> <td><span class="require">', $line = 52, $out += $escape(bus.require), 
                $out += "</span></td> ", $line = 53, isFrame || ($out += ' <td> <a class="cursor T-arrange-item T-action R-right" data-right=\'1480001\'> 安排 </a> <a class="cursor T-view T-action"><label class="padding-right20">|</label> 查看</a> </td> ', 
                $line = 60), $out += " </tr> ", $line = 62;
            }), $out += ' </tbody> </table> <div class="clearfix"></div> <div class="row pageMode"> <div class="col-xs-6"> <small>共计 ', 
            $line = 68, $out += $escape(recordSize), $out += ' 条记录</small> </div> <div class="col-xs-6"> <div class="T-pagenation text-right"> </div> </div> </div> <div class="text-center"> ', 
            $line = 76, isFrame || ($out += ' <button class="btn btn-sm btn-success w-150 R-right T-merge-arrange" ', 
            $line = 77, canMergeArrange || ($out += ' disabled="disabled" ', $line = 77), $out += ' data-right="1130004"> <i class="ace-icon fa fa-user-plus"></i> 统一安排 </button> ', 
            $line = 80), $out += " </div> ", new String($out);
        } catch (e) {
            throw {
                filename: $filename,
                name: "Render Error",
