/*TMODJS:{"debug":true,"version":15,"md5":"8e955506655c7681d5bc55f679cb1e52"}*/
    ...
    return require("../../../template")("arrange/arrangeCenter/bookingView/ticketList", function($data, $filename) {
        try {
            ...
            $line = 19, $each(bookingOrderList, function(rs) {
                $out += ' <tr data-id="', $line = 20, $out += $escape(rs.id), $out += '" data-start-time="', 
                $line = 21, $out += $escape(rs.startTime), $out += '" data-starting-city="', $line = 22, 
                $out += $escape(rs.startingCity), $out += '" data-arrive-city="', $line = 23, $out += $escape(rs.arriveCity), 
                $out += '" data-seat-level="', $line = 24, $out += $escape(rs.seatLevel), $out += '" data-member-count="', 
                $line = 25, $out += $escape(rs.memberCount), $out += '"> <td>', $line = 26, $out += $escape(rs.orderNumber), 
                $out += "</td> <td>", $line = 27, $out += $escape($helpers.getTicketText(rs.type)), 
                $out += "</td> <td>", $line = 28, $out += $escape($helpers.dateFormat(rs.startTime, "yyyy-MM-dd hh:mm")), 
                $out += "</td> <td>", $line = 29, $out += $escape(rs.startingCity), $out += " - ", 
                $line = 29, $out += $escape(rs.arriveCity), $out += "</td> <td>", $line = 30, $out += $escape(rs.seatLevel), 
                $out += "</td> <td>", $line = 31, $out += $escape(rs.memberCount), $out += "</td> <td>", 
                $line = 32, $out += $escape(rs.required), $out += "</td> <td>", $line = 33, $out += $string(rs.arrangeResult || "-"), 
                $out += "</td> <td>", $line = 34, $out += $escape(rs.outOPUserName), $out += "</td> <td> ", 
                $line = 36, 1 == rs.status ? ($out += ' <i class="ace-icon fa fa-check green bigger-130"></i>已完成 ', 
                $line = 38) : ($out += ' <i class="ace-icon fa fa-times red bigger-125"></i>未完成 ', 
                $line = 40), $out += ' </td> <td> <a billstatus="-1" class="cursor T-action T-plan R-right" data-right="1480009" title="安排"> 安排 </a> <a class="cursor T-action T-view" title="查看"> <label class="padding-right20">|</label> 查看 </a> </td> </tr> ', 
                $line = 50;
