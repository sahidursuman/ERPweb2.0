/*TMODJS:{"debug":true,"version":36,"md5":"cc741d68aba5aa6d310a92bbe1590d8d"}*/
    ...
    return require("../../../template")("arrange/arrangeCenter/bookingView/busList", function($data, $filename) {
        try {
            ...
            $line = 14, $each(bookingOrderList, function(rs) {
                $out += ' <tr data-id="', $line = 15, $out += $escape(rs.id), $out += '" data-start-use-time="', 
                $line = 16, $out += $escape($helpers.dateFormat(rs.startUseTime, "yyyy-MM-dd")), 
                $out += '" data-end-use-time="', $line = 17, $out += $escape($helpers.dateFormat(rs.endUseTime, "yyyy-MM-dd")), 
                $out += '" > <td>', $line = 19, $out += $escape(rs.orderNumber), $out += "</td> <td>", 
                $line = 20, $out += $escape($helpers.dateFormat(rs.startUseTime, "yyyy-MM-dd")), 
                $out += " - ", $line = 20, $out += $escape($helpers.dateFormat(rs.endUseTime, "yyyy-MM-dd")), 
                $out += "</td> <td>", $line = 21, $out += $escape(rs.required), $out += "</td> <td>", 
                $line = 22, $out += $string(rs.arrangeResult || "-"), $out += "</td> <td>", $line = 23, 
                $out += $escape(rs.outOPUserName), $out += "</td> <td> ", $line = 25, 1 == rs.status ? ($out += ' <i class="ace-icon fa fa-check green bigger-130"></i>已完成 ', 
                $line = 27) : ($out += ' <i class="ace-icon fa fa-times red bigger-125"></i>未完成 ', 
                $line = 29), $out += ' </td> <td> <a billstatus="-1" class="cursor T-action T-plan R-right" data-right="1480006" title="安排"> 安排 </a> <a class="cursor T-action T-view" title="查看"><label class="padding-right20">|</label> 查看 </a> </td> </tr> ', 
                $line = 39;
            }), $out += ' </tbody> </table> <div class="clearfix"></div> <div class="row pageMode"> <div class="col-xs-6"> <small>共计 ', 
            $line = 45, $out += $escape(recordSize), $out += ' 条记录</small> </div> <div class="col-xs-6"> <div class="T-pagenation text-right"> </div> </div> </div> ', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
