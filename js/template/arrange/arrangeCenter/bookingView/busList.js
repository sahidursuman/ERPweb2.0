/*TMODJS:{"debug":true,"version":26,"md5":"46080be7db2baf778fb7c7023324557b"}*/
    ...
    return require("../../../template")("arrange/arrangeCenter/bookingView/busList", function($data, $filename) {
        try {
            ...
            $line = 14, $each(bookingOrderList, function(rs) {
                $out += ' <tr data-id="', $line = 15, $out += $escape(rs.id), $out += '" data-startUseTime = "', 
                $line = 15, $out += $escape($helpers.dateFormat(rs.startUseTime, "yyyy-MM-dd")), 
                $out += '" data-endUseTime = "', $line = 15, $out += $escape($helpers.dateFormat(rs.endUseTime, "yyyy-MM-dd")), 
                $out += '"> <td>', $line = 16, $out += $escape(rs.orderNumber), $out += "</td> <td>", 
                ...
                $line = 26), $out += ' </td> <td> <a billstatus="-1" class="cursor T-action T-plan R-right" data-right="1480006" title="安排"> 安排 </a> <a class="cursor T-action T-view" title="查看"><label class="padding-right20">|</label> 查看 </a> </td> </tr> ', 
                $line = 36;
