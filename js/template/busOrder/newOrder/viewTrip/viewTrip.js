/*TMODJS:{"debug":true,"version":24,"md5":"335b24beed2626586f37f84d320e702f"}*/
    ...
    return require("../../../template")("busOrder/newOrder/viewTrip/viewTrip", function($data, $filename) {
        try {
            ...
            $line = 50, $each(order.otherFeeList, function(rs) {
                $out += ' <div> <label class="lineblock width150">金额：', $line = 52, $out += $escape(rs.fee), 
                $out += '<input class="T-clacFee" name="fee" type="hidden" value="', $line = 52, 
                $out += $escape(rs.fee), $out += '"></label> <label> 备注：', $line = 53, $out += $escape(rs.remark), 
                $out += "</label> </div> ", $line = 55;
            }), $out += " </div> ", $line = 57, "1" == order.isShowArrangeInfoColumn && ($out += ' <div class="col-xs-12 padding-b10 mar-b10 border-bottom"> <h5 class="color-999">派车信息</h5> <label> ', 
            $line = 61, order.arrangeInfoList.length ? ($out += " ", $line = 62, $each(order.arrangeInfoList, function(arrangeInfo) {
                $out += " ", $line = 63, $out += $escape(arrangeInfo.driverName), $out += " ", $line = 63, 
                $out += $escape(arrangeInfo.driverMobileNumber), $out += " ", $line = 63, $out += $escape(arrangeInfo.licenceNumber), 
                $out += " <br> ", $line = 64;
            }), $out += " ", $line = 65) : ($out += ' <span class="red">暂未派车</span> ', $line = 67), 
            $out += " </label> </div> ", $line = 70), $out += ' <div class="T-touristList"> <div class="col-xs-12"> <p>人数统计：<span class="T-allPeopleText">0大0小</span></p> <p>费用统计：<span class="T-allFeeText">0*(0+0)+0 = <span class="font-size20 red">0</span>元</span></p> </div> </div> </div>', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
