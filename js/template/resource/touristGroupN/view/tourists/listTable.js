/*TMODJS:{"debug":true,"version":32,"md5":"75424aafb0a9b5ce71e8fddfe302774d"}*/
    ...
    return require("../../../../template")("resource/touristGroupN/view/tourists/listTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(touristGroupList, function(touristGroup) {
                ...
                $line = 36, 1 == touristGroup.innerFlag && 1 == touristGroup.status ? ($out += ' <a class="T-action T-outer-turn cursor"> <label class="padding-right20">|</label> <span>外转</span> </a> ', 
                $line = 41) : 1 != touristGroup.innerFlag && ($out += ' <a data-entity-id="', $line = 42, 
                $out += $escape(touristGroup.id), $out += '" class="T-action R-right ', $line = 42, 
                touristGroup.canEdit && ($out += "T-edit", $line = 42), $out += ' cursor" ', $line = 42, 
                touristGroup.canEdit || ($out += 'style="color:#bbb;" title="', $line = 42, $out += $escape(touristGroup.editTitle), 
                $out += '"', $line = 42), $out += ' data-right="1470002"> <label class="padding-right20">|</label> <span>编辑</span> </a> <a data-entity-id="', 
