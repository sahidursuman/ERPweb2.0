/*TMODJS:{"debug":true,"version":12,"md5":"e70d20ef6dd09dbae54ad5086dfedbe8"}*/
define(function(require) {
    return require("../../../../../template")("resource/touristGroupN/view/tourists/view/viewOuterTurn", function($data, $filename) {
        try {
            var $utils = this, $helpers = $utils.$helpers, $line = 0, lineData = $data.lineData, $escape = $utils.$escape, transferPartnerAgency = $data.transferPartnerAgency, remark = $data.remark, needPayAllMoney = $data.needPayAllMoney, currentNeedPayMoney = $data.currentNeedPayMoney, isNowIncome = $data.isNowIncome, outTransferFee = $data.outTransferFee, $each = $utils.$each, $out = ($data.rs, 
            $data.$index, "");
            ...
            $line = 9), $out += '</div> </div> <div class="col-xs-12 mar-t-20"> <div class="col-xs-1 text-right no-padding-right control-label">同行</div> <div class="col-xs-3 control-label">', 
            $line = 13, $out += $escape(transferPartnerAgency), $out += '</div> </div> <div class="col-xs-12 mar-t-20"> <div class="col-xs-1 text-right no-padding-right control-label">备注</div> <div class="col-xs-10 control-label">', 
            $line = 17, $out += $escape(remark), $out += '</div> </div> <div class="col-xs-12 mar-t-10"> <div class="col-xs-1 text-right no-padding-right control-label">应付</div> <div class="col-xs-3 control-label">', 
            $line = 21, $out += $escape(needPayAllMoney), $out += '</div> <div class="col-xs-1 text-right no-padding-right control-label">计划现收</div> <div class="col-xs-2 control-label">', 
            $line = 23, $out += $escape(currentNeedPayMoney), $out += '</div> <div class="col-xs-1 text-right no-padding-right control-label">对方现收</div> <div class="col-xs-1"> <input type="checkbox" disabled class="ace"', 
            $line = 26, 1 == isNowIncome && ($out += " checked", $line = 26), $out += '> <label class="lbl"></label> </div> </div> <div class="col-xs-12 mar-t-20"> <table class="table table-striped table-new table-bordered table-hover"> <thead> <tr> <th>费用项</th> <th>数量</th> <th>单价</th> <th>金额</th> <th>说明</th> </tr> </thead> <tbody class="T-fee-list"> ', 
            $line = 42, outTransferFee && outTransferFee.length > 0 && ($out += " ", $line = 43, 
            $each(outTransferFee, function(rs) {
                $out += " <tr> <td>", $line = 45, $out += $escape($helpers.getFeeItemText(rs.type)), 
                $out += "</td> <td>", $line = 46, $out += $escape(rs.count), $out += "</td> <td>", 
                $line = 47, $out += $escape(rs.price), $out += "</td> <td>", $line = 48, $out += $escape(rs.price * rs.count), 
                $out += "</td> <td>", $line = 49, $out += $escape(rs.remark), $out += "</td> </tr> ", 
                $line = 51;
            }), $out += " ", $line = 52), $out += ' </tbody> </table> </div> <div class="col-xs-12 mar-t-20 mar-b-10 text-center"> <button class="btn btn-xs btn-default T-btn-close btn-primary w100"> <i class="ace-icon fa fa-times"></i> 关闭 </button> </div> </div> </div>', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
