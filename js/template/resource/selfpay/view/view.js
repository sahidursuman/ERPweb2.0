/*TMODJS:{"debug":true,"version":271,"md5":"0495a7dd38e88e262e9bbce6f6e368c7"}*/
    ...
    return require("../../../template")("resource/selfpay/view/view", function($data, $filename) {
        try {
            ...
            $line = 69, $each(selfpay.selfPayItemList, function(shopPolicy) {
                ...
                $out += ' </td> <td class="rangeTimeWidth" > <div> 日常价格 </div> ', $line = 85, $each(shopPolicy.SelfPayRebateList, function(hopTimeArea) {
                    $out += " <div> ", $line = 87, $out += $escape($helpers.dateFormat(hopTimeArea.startTime, "yyyy-MM-dd")), 
                    $out += "至", $line = 87, $out += $escape($helpers.dateFormat(hopTimeArea.endTime, "yyyy-MM-dd")), 
                    $out += " </div> ", $line = 89;
                }), $out += ' </td> <td class="no-padding"> ', $line = 92, $out += $escape(shopPolicy.normalInnerPrice), 
                $out += "<br> ", $line = 93, $each(shopPolicy.SelfPayRebateList, function(hopTimeArea) {
                    $out += " ", $line = 94, $out += $escape(hopTimeArea.price), $out += "<br> ", $line = 95;
                }), $out += ' </td> <td class="no-padding"> ', $line = 99, $out += $escape(shopPolicy.normalMarketPrice), 
                $out += "<br> ", $line = 100, $each(shopPolicy.SelfPayRebateList, function(hopTimeArea) {
                    $out += " ", $line = 101, $out += $escape(hopTimeArea.marketPrice), $out += "<br> ", 
                    $line = 102;
                }), $out += ' </td> <td class="no-padding"> <label class="col-sm-12 control-label center"> ', 
                $line = 106, $out += $escape($helpers.toFixed(100 * shopPolicy.normalGuideRate)), 
                $out += " </label> ", $line = 108, $each(shopPolicy.SelfPayRebateList, function(hopTimeArea) {
                    $out += ' <label class="col-sm-12 control-label center"> ', $line = 110, $out += $escape($helpers.toFixed(100 * hopTimeArea.guideRate)), 
                    $out += " </label> ", $line = 112;
                }), $out += ' </td> <td class="no-padding"> <label class="col-sm-12 control-label center"> ', 
                $line = 116, $out += $escape($helpers.toFixed(100 * shopPolicy.normalTravelAgencyRate)), 
                $out += " </label> ", $line = 118, $each(shopPolicy.SelfPayRebateList, function(hopTimeArea) {
                    $out += ' <label class="col-sm-12 control-label center"> ', $line = 120, $out += $escape($helpers.toFixed(100 * hopTimeArea.travelAgencyRate)), 
                    $out += " </label> ", $line = 122;
                }), $out += ' </td> <td class="remarkWidth">', $line = 124, $out += $escape(shopPolicy.remark), 
                $out += "</td> </tr> ", $line = 126;
