/*TMODJS:{"debug":true,"version":115,"md5":"3b443f623ef56155548afa78209b75a1"}*/
    ...
    return require("../../../template")("resource/hotel/view/view", function($data, $filename) {
        try {
            ...
            $line = 86, $each(hotel.hotelRoomList, function(room) {
                ...
                $line = 88, $out += $escape(room.type), $out += '</td> <td class="rangeTimeWidth"> <div>日常</div> ', 
                $line = 91, $each(room.priceList, function(price) {
                    ...
                    $out += " </div> ", $line = 95;
                }), $out += " </td> <td> <div> ", $line = 99, $out += $escape(room.normalMarketPrice), 
                $out += " </div> ", $line = 101, $each(room.priceList, function(price) {
                    $out += " <div> ", $line = 103, $out += $escape(price.marketPrice), $out += " </div> ", 
                    $line = 105;
                }), $out += " </td> <td> <div> ", $line = 110, $out += $escape(room.normalInnerPrice), 
                $out += " </div> ", $line = 112, $each(room.priceList, function(price) {
                    $out += " <div> ", $line = 114, $out += $escape(price.contractPrice), $out += " </div> ", 
                    $line = 116;
                ...
                $out += $escape(room.broadband), $out += " </td> <td> ", $line = 145, $out += $escape(room.areaSize), 
                $out += " </td> <td> ", $line = 148, $out += $escape(room.guestNumber), $out += ' </td> <td class="remarkWidth"> ', 
