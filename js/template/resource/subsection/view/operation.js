/*TMODJS:{"debug":true,"version":387,"md5":"0147d38b29b160d15bd4c3cb705a822f"}*/
    ...
    return require("../../../template")("resource/subsection/view/operation", function($data, $filename) {
        try {
            ...
            $line = 32, $each(subtGroupList, function(subList) {
                ...
                $line = 37, 1 == subList.status && ($out += "chooseLineProduct", $line = 37), $out += ' col-sm-12" name="lineProduct" ', 
                $line = 37, 1 != subList.status && ($out += 'readonly="readonly"', $line = 37), 
                $out += ' type="text" value="', $line = 37, $out += $escape(subList.lineProduct.name), 
                ...
                $line = 41, 1 == subList.status && ($out += "datepicker", $line = 41), $out += ' col-sm-12" name="startTime" type="text" ', 
                ...
                $out += '" /></td> <td> ', $line = 43, 0 == mark ? ($out += " ", $line = 44, 1 == subList.status ? ($out += ' <input type="radio" name="operateCurrentNeedPayMoney" ', 
                $line = 45, 1 == subList.operateCurrentNeedPayMoney && ($out += 'checked="checked"', 
                $line = 45), $out += " /> ", $line = 46) : ($out += " - ", $line = 48), $out += " ", 
                $line = 49) : 1 == mark && 1 == subList.operateCurrentNeedPayMoney ? ($out += " 本段现收 ", 
                $line = 51) : ($out += " - ", $line = 53), $out += ' <input type="hidden" name="operateCurrentNeedPayMoney" value="', 
                $line = 54, $out += $escape(subList.operateCurrentNeedPayMoney), $out += '" /> </td> <td> ', 
                $line = 57, 0 == subList.status ? ($out += " 已发团 ", $line = 59) : 1 == subList.status ? ($out += " 未分团 ", 
                $line = 61) : 2 == subList.status ? ($out += " 已分团 ", $line = 63) : 3 == subList.status ? ($out += " 已转客 ", 
                $line = 65) : 4 == subList.status ? ($out += " 已取消 ", $line = 67) : 5 == subList.status ? ($out += " 已分段 ", 
                $line = 69) : 6 == subList.status && ($out += " 已内转 ", $line = 71), $out += ' </td> <td> <div class="hidden-sm hidden-xs btn-group"> <a data-entity-id="', 
                $line = 75, $out += $escape(subList.id), $out += '" class=" ', $line = 75, 1 == subList.status && ($out += "btn-operation-delete", 
                $line = 75), $out += ' cursor" ', $line = 75, 1 != subList.status && ($out += 'style="color: #bbb;"', 
                $line = 75), $out += "> 删除 </a> </div> </td> </tr>", $line = 80), $line = 80;
            ...
            $line = 86, 1 == mark && ($out += 'data-entity-mark="1"', $line = 86), $out += '> <i class="ace-icon fa fa-check"></i> 保存 </button> </div> </div>', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
