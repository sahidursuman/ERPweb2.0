/*TMODJS:{"debug":true,"version":390,"md5":"9bd12d074869f004c454d57a3fe47152"}*/
    ...
    return require("../../../template")("resource/subsection/view/operation", function($data, $filename) {
        try {
            ...
            $line = 33, $each(subtGroupList, function(subList) {
                ...
                $line = 44, 1 == subList.operateCurrentNeedPayMoney && ($out += 'checked="checked"', 
                $line = 44), $out += " /> </td> <td> ", $line = 47, 0 == subList.status ? ($out += " 已发团 ", 
                $line = 49) : 1 == subList.status ? ($out += " 未分团 ", $line = 51) : 2 == subList.status ? ($out += " 已分团 ", 
                $line = 53) : 3 == subList.status ? ($out += " 已转客 ", $line = 55) : 4 == subList.status ? ($out += " 已取消 ", 
                $line = 57) : 5 == subList.status ? ($out += " 已分段 ", $line = 59) : 6 == subList.status && ($out += " 已内转 ", 
                $line = 61), $out += ' </td> <td> <div class="hidden-sm hidden-xs btn-group"> <a data-entity-id="', 
                $line = 65, $out += $escape(subList.id), $out += '" class=" ', $line = 65, 1 == subList.status && ($out += "btn-operation-delete", 
                $line = 65), $out += ' cursor" ', $line = 65, 1 != subList.status && ($out += 'style="color: #bbb;"', 
                $line = 65), $out += "> 删除 </a> </div> </td> </tr>", $line = 70), $line = 70;
            ...
            $line = 76, 1 == mark && ($out += 'data-entity-mark="1"', $line = 76), $out += '> <i class="ace-icon fa fa-check"></i> 保存 </button> </div> </div>', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
