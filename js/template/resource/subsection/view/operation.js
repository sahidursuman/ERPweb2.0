/*TMODJS:{"debug":true,"version":416,"md5":"1abf3229605e3b10c429c9691d1a39be"}*/
define(function(require) {
    return require("../../../template")("resource/subsection/view/operation", function($data, $filename) {
        try {
            ...
            $out += '</span></label> <label class="control-label col-xs-2">人数合计: ', $line = 11, 
            $out += $escape(ptGroup.adultCount), $out += "大", $line = 11, $out += $escape(ptGroup.childCount), 
            $out += '小</label> <label class="control-label col-xs-2">来源: ', $line = 12, $out += $escape(ptGroup.partnerAgency.travelAgencyName), 
            $out += '</label> <label class="control-label col-xs-2">联系人: ', $line = 13, $out += $escape(ptGroup.contactMember.name), 
            $out += '</label> <label class="control-label col-xs-2">联系电话: ', $line = 14, $out += $escape(ptGroup.contactMember.mobileNumber), 
            $out += '</label> </div> <div class="form-group"> <button class="btn btn-sm btn-success T-btn-operation-add" data-mark="', 
            $line = 17, $out += $escape(mark), $out += '"> <i class="ace-icon fa fa-plus"></i>新增分段</button> </div> <div class="form-group"> <table class="table table-striped table-bordered table-hover"> <thead> <tr class="bg-blur"> <th>线路产品</th> <th>针对客源</th> <th>行程天数</th> <th>出游日期</th> <th>本段现收团款</th> <th>状态</th> <th>操作</th> </tr> </thead> <tbody class="T-subsectionOperationTbody">', 
