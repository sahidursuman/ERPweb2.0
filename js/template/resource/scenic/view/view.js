/*TMODJS:{"debug":true,"version":80,"md5":"c38a5c405abaf827951e810627ae05f5"}*/
    ...
    return require("../../../template")("resource/scenic/view/view", function($data, $filename) {
        try {
            ...
            $line = 63, $each(scenic.scenicItemList, function(item) {
                ...
                $line = 65, $out += $escape(item.name), $out += '</td> <td class="rangeTimeWidth"> <div> 日常 </div> ', 
                $line = 70, $each(item.priceList, function(price) {
                    ...
                    $out += " </div> ", $line = 74;
                }), $out += " </td> <td> <div> ", $line = 78, $out += $escape(item.normalInnerPrice), 
                $out += " </div> ", $line = 80, $each(item.priceList, function(price) {
                    $out += " <div> ", $line = 82, $out += $escape(price.contractPrice), $out += " </div> ", 
                    $line = 84;
