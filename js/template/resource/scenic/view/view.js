/*TMODJS:{"debug":true,"version":61,"md5":"994fca8f99d53dae321e1e7c800f42a8"}*/
define(function(require) {
    return require("../../../template")("resource/scenic/view/view", function($data, $filename) {
        try {
            ...
            $line = 27, $out += $escape(scenic.faxNumber), $out += '</td> </tr> <tr> <td class="style-myself">景区所在省市：</td> <td class="styleOne-self" colspan="3">', 
            ...
            $line = 63, $each(scenic.scenicItemList, function(item) {
                $out += ' <tr data-entity-id="', $line = 64, $out += $escape(item.id), $out += '"> <td>', 
                $line = 65, $out += $escape(item.name), $out += '</td> <td class="rangeTimeWidth"> ', 
                $line = 67, $each(item.priceList, function(price) {
                    $out += " <div> ", $line = 69, $out += $escape($helpers.dateFormat(price.startTime, "yyyy-MM-dd")), 
                    $out += "至", $line = 69, $out += $escape($helpers.dateFormat(price.endTime, "yyyy-MM-dd")), 
                    $out += " </div> ", $line = 71;
                }), $out += " </td> <td> ", $line = 74, $each(item.priceList, function(price) {
                    $out += " <div> ", $line = 76, $out += $escape(price.contractPrice), $out += " </div> ", 
                    $line = 78;
                }), $out += ' </td> <td class="remarkWidth"> ', $line = 81, $out += $escape(item.remark), 
                $out += " </td> </tr> ", $line = 84;
