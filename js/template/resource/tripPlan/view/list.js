/*TMODJS:{"debug":true,"version":487,"md5":"d2531ba2f3060c6f7400c274e38bb7e1"}*/
define(function(require) {
    return require("../../../template")("resource/tripPlan/view/list", function($data, $filename) {
        try {
            ...
            return $out += '<table class="table table-striped table-bordered table-hover T-showHighLight tripPlanViewList"> <thead> <tr class="bg-blur"> <th>团号</th> <th>团队人数</th> <th>导</th> <th>车</th> <th>险</th> <th>餐</th> <th>房</th> <th>景</th> <th>票</th> <th>购</th> <th>娱</th> <th>它</th> <th>责任部门</th> <th>状态</th> <th>操作</th> </tr> </thead> <tbody> <input type="hidden" name="travelAgencyName" value="', 
            $line = 22, $out += $escape(travelAgencyName), $out += '"> ', $line = 23, $each(tripPlanList, function(tripPlan) {
                $out += ' <tr data-entity-id="', $line = 24, $out += $escape(tripPlan.id), $out += '"> <td class="h-touristGroupInfo"> <p class="h-orderNumber">', 
                $line = 26, $out += $escape(tripPlan.tripNumber), $out += '</p> <p class="hct-absolute-group h-lineName"> <span class="hct-span-left"> ', 
                $line = 29, 0 != tripPlan.tripPlanDetails.length ? ($line = 29, $out += $escape(tripPlan.tripPlanDetails[0].lineProductName), 
                $line = 29) : ($line = 29, tripPlan.lineProduct && ($line = 29, $out += $escape(tripPlan.lineProduct.name), 
                $line = 29), $line = 29), $out += " </span> ", $line = 31, tripPlan.tripPlanDetails.length > 1 && ($out += ' <label class="T-action T-showLineInfo hct-button-right"> <button class="btn btn-success btn-sm btn-white show" index="0"> <i class="fa fa-plus bigger-110 icon-only"></i> </button> </label> ', 
                $line = 37), $out += ' </p> <p class="h-startTime"><span>', $line = 39, 0 != tripPlan.tripPlanDetails.length ? ($line = 39, 
                $out += $escape($helpers.dateFormat(tripPlan.tripPlanDetails[0].startTime, "yyyy.MM.dd")), 
                $line = 39) : ($line = 39, $out += $escape($helpers.dateFormat(tripPlan.startTime, "yyyy.MM.dd")), 
                $line = 39), $out += "</span>-<span>", $line = 39, 0 != tripPlan.tripPlanDetails.length ? ($line = 39, 
                $out += $escape($helpers.dateFormat(tripPlan.tripPlanDetails[0].endTime, "yyyy.MM.dd")), 
                $line = 39) : ($line = 39, $out += $escape($helpers.dateFormat(tripPlan.endTime, "yyyy.MM.dd")), 
                $line = 39), $out += '</span></p> <p class="h-memberName">', $line = 40, tripPlan.dutyOPUser && ($line = 40, 
                $out += $escape(tripPlan.dutyOPUser.realName), $line = 40), $out += "</p> </td> <td>", 
                $line = 44, $out += $escape(tripPlan.touristAdultCount), $out += "大", $line = 44, 
                ...
                $line = 65, tripPlan.businessGroup && ($line = 65, $out += $escape(tripPlan.businessGroup.name), 
                $line = 65), $out += "</td> ", $line = 66, 0 == tripPlan.status ? ($out += ' <td class="warning-text">待确定</td> ', 
                $line = 68) : 1 == tripPlan.status ? ($out += ' <td class="success-text">已发团</td> ', 
