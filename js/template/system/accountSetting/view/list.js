/*TMODJS:{"debug":true,"version":110,"md5":"33d1705646e38ee967dbc33b36ff9cea"}*/
define(function(require) {
    return require("../../../template")("system/accountSetting/view/list", function($data, $filename) {
        try {
            ...
            $line = 40, $each(result, function(result) {
                $out += " <tr> <td>", $line = 42, $out += $escape(result.applyNumber), $out += "</td> <td>", 
                $line = 43, $out += $escape(result.name), $out += "</td> <td>", $line = 44, $out += $escape(result.mobileNumber), 
                $out += "</td> <td>", $line = 45, $out += $escape(result.applyMoney), $out += "</td> <td>", 
                $line = 46, $out += $escape(result.approvedMoney), $out += "</td> <td>", $line = 47, 
                $out += $escape(result.createTime), $out += "</td> <td>", $line = 48, $out += $escape(result.examineDate), 
                $out += "</td> <td> ", $line = 50, 0 == result.applyStatus ? ($out += " 未审核 ", $line = 52) : 1 == result.applyStatus ? ($out += " 已通过 ", 
                $line = 54) : 2 == result.applyStatus && ($out += " 未通过 ", $line = 56), $out += " </td> </tr> ", 
                $line = 59;
            }), $out += ' </tbody> </table> <div class="row pageMode"> <div class="col-xs-6"> <small>共计 ', 
            $line = 65, $out += $escape(searchParam.totalCount), $out += ' 条记录</small> </div> <div class="col-xs-6"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div> </div>  <div id="account-bankAccount" class="account-bankAccount tab-pane fade" > <div class="T-bankAccount-content"> </div> </div> </div> </div> ', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
