/*TMODJS:{"debug":true,"version":126,"md5":"0e7ecc0be944c8c0254a1ad7ea2500e9"}*/
    ...
    return require("../../../template")("system/accountSetting/view/list", function($data, $filename) {
        try {
            ...
            $line = 42, $each(result, function(result) {
                $out += ' <tr data-applyId="', $line = 43, $out += $escape(result.applyId), $out += '"> <td>', 
                $line = 44, $out += $escape(result.applyNumber), $out += "</td> <td>", $line = 45, 
                $out += $escape(result.name), $out += "</td> <td>", $line = 46, $out += $escape(result.mobileNumber), 
                $out += "</td> <td>", $line = 47, $out += $escape(result.applyMoney), $out += "</td> <td>", 
                $line = 48, $out += $escape(result.approvedMoney), $out += "</td> <td>", $line = 49, 
                $out += $escape(result.createTime), $out += "</td> <td>", $line = 50, $out += $escape(result.examineDate), 
                $out += "</td> <td> ", $line = 52, 0 == result.applyStatus ? ($out += " 待审核 ", $line = 54) : 1 == result.applyStatus ? ($out += " 已通过 ", 
