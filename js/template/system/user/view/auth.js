/*TMODJS:{"debug":true,"version":359,"md5":"e5c59e49512b585ede32b429dc25769a"}*/
define(function(require) {
    return require("../../../template")("system/user/view/auth", function($data, $filename) {
        try {
            ...
            return $out += '<div class="col-xs-12 T-update-auth userUpdate-auth" id="userSetAuth" style="width: 850px;"> <div class="page-header" style="font-weight: bold;font-size:14px;"> 【当前用户：', 
            ...
            }), $out += ' </table> <div class="space-10"></div> <button class="btn btn-block btn-primary T-saveAuth" style="margin-bottom:10px;"> <i class="ace-icon fa fa-check"></i> 提交信息 </button> </form> <div class="userAuthWidth"> ', 
            $line = 77, $each(listMenu, function(menu, menuIndex) {
                $out += " ", $line = 78, "工作台" != menu.name && "消息中心" != menu.name && ($out += ' <table class="table table-striped table-bordered table-hover"> ', 
                $line = 80, 0 == menuIndex && ($out += ' <thead> <th class="th-border">功能模块</th> <th class="th-border" colspan="5">权限</th> </thead> ', 
                $line = 85), $out += ' <ul class="userAuth"> <li class="meun" data-toid="D-', $line = 87, 
                $out += $escape(menu.id), $out += '">', $line = 87, $out += $escape(menu.name), 
                $out += "</li> ", $line = 88), $out += " ", $line = 89;
