/*TMODJS:{"debug":true,"version":61,"md5":"31887c1cbe0dcee8992cc0c0917213af"}*/
define(function(require) {
    return require("../../../template")("system/department/view/auth", function($data, $filename) {
        try {
            ...
            return $out += '<div class="col-xs-12" id="userSetAuthd"> <div class="page-header" style="font-weight: bold;font-size:14px;"> 【当前用户：', 
            ...
            $line = 15, $each(menuList, function(menu) {
                ...
                $out += '" class="pmodule" />', $line = 18, $out += $escape(menu.name), $out += '</div></td> <td colspan="3">操作权限</td> <td colspan="4">权限范围</td> </tr> ', 
                $line = 23, $each(menuList, function(menu2, index) {
                    ...
                    $out += '" data-id="', $line = 37, $out += $escape(menu2.id), $out += '"> <td><input type="checkbox" name="" value="', 
                    $line = 38, $out += $escape(menu2.id), $out += '|s" class="smodule" />', $line = 38, 
                    $out += $escape(menu2.name), $out += '</td> <td><input type="checkbox" name="" value="', 
                    $line = 39, $out += $escape(menu2.id), $out += '|a" />添加</td> <td><input type="checkbox" name="" value="', 
                    $line = 40, $out += $escape(menu2.id), $out += '|e" />修改</td> <td><input type="checkbox" name="" value="', 
                    $line = 41, $out += $escape(menu2.id), $out += '|d" />删除</td> <td><input type="radio" name="auTy', 
                    $line = 42, $out += $escape(menu2.id), $out += '" value="auTy1">个人</td> <td><input type="radio" name="auTy', 
                    $line = 43, $out += $escape(menu2.id), $out += '" value="auTy2">子部门</td> <td><input type="radio" name="auTy', 
                    $line = 44, $out += $escape(menu2.id), $out += '" value="auTy3">业务部门</td> <td><input type="radio" name="auTy', 
                    $line = 45, $out += $escape(menu2.id), $out += '" value="auTy4">公司</td> </tr> ', 
                    $line = 47), $out += " ", $line = 48;
