/*TMODJS:{"debug":true,"version":14,"md5":"59fe817ea17c6caf8b280f6477ae9686"}*/
define(function(require) {
    return require("../../../template")("system/department/view/updateGroup", function($data, $filename) {
        try {
            ...
            $line = 7, $out += $escape(businessGroup.name), $out += '</label> <input name="businessGroupId" type="hidden" value="', 
            $line = 8, $out += $escape(businessGroup.id), $out += '" /> </div> </div> <div class="form-group"> <label class="col-sm-3 control-label no-padding-right"><span class="necessary">*</span>子部门名称:</label> <div class="col-sm-4"> <input type="hidden" name="groupId" value="', 
            $line = 14, $out += $escape(group.id), $out += '" /> <input type="text" name="name" class="col-sm-12" value="', 
            $line = 15, $out += $escape(group.name), $out += '" /> </div> </div> <div class="space-10"></div> <button class="btn btn-block btn-primary btn-submit-group"> <i class="ace-icon fa fa-check"></i> 提交信息 </button> </form> </div> ', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
