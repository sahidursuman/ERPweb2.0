				...
				success:function(data){
					...
					if(result){
						...
						var updateBusinessGroupLayer = layer.open({
						    ...
						    content: html,
						    success:function(){
								var $obj = $(".addDepartment .businessMainForm");
								// 设置表单验证
								var validator = rule.check($('.addDepartment'));

								$obj.find(".btn-submit-department").click(function(){
									alert();
		...
		addBusiness:function() {
			var html = addTemplate();
						var addBusinessGroupLayer = layer.open({
						    ...
						    success:function(){
						    	//表单验证
						    	var validator=rule.check($(".businessMainForm"));
				...
				success:function(data){
					...
					if(result){
						...
						var addGroupLayer = layer.open({
						    ...
						    success:function(){
								//表单验证
								var validator=rule.check($(".groupMainForm"));
						...
						var updateAuth = layer.open({
						    ...
						    success:function(){
						    	...
						    	for(var i=0; i < data.userAuthList.length; i ++) {
						    		...
						    		$('#userSetAuthd .userMainForm .checkibox'+id).find("input[value='auTy" + authType + "']").attr("checked","checked");
						    		
						    		$('#userSetAuthd .userMainForm .checkibox'+id).find("input[value='" + id + "|s']").attr("checked","checked");
						    		if(a) {
						    			$('#userSetAuthd .userMainForm .checkibox'+id).find("input[value='" + id + "|a']").attr("checked","checked");
						    		}
									if(e) {
										$('#userSetAuthd .userMainForm .checkibox'+id).find("input[value='" + id + "|e']").attr("checked","checked");
									}
									if(d) {
										$('#userSetAuthd .userMainForm .checkibox'+id).find("input[value='" + id + "|d']").attr("checked","checked");
									}
						    	}
						    	...
						    	for(var i=0; i < data.menuList.length; i ++) {
						    		...
						    		if(menu.pid == 0) {
						    			var menuId = menu.id;
						    			var trs = $('#userSetAuthd .userMainForm .pmodule'+menuId);
						    	...
						    	//总选框
						    	$("#userSetAuthd .userMainForm .pmodule").click(function() {
						    		var id = $(this).val();
						    		var trs = $('#userSetAuthd .pmodule' + id);
						    	...
						    	//复选框的单选和反选  
						    	$("#userSetAuthd .userMainForm .smodule").click(function(){
						    		if($(this).is(":checked")){	
						    			//复选框
						    	...
						    	//列全选
						    	$("#userSetAuthd .sAll").click(function() {
						    		var td = $(this).attr("td");
						    		var id = $(this).attr("data-id");
						    		$("#userSetAuthd").find(".pmodule" + id).each(function() {
						    			$($(this).find('td')[td]).find('input').prop("checked",true);
						    			$($(this).find('td')[0]).find('input').prop("checked",true);
						    		});
						    	});
						    	
						    	$("#userSetAuthd .authId").find('input[type=checkbox]').change(function() {
						    		if($(this).is(":checked")){	
								...
								$("#userSetAuthd .userMainForm .btn-submit-group").click(function(){
									//组装json
									...
									json += "]";
									json = "menuAuth="+json
									var userId = $('#userSetAuthd .userMainForm #userId').val();
									$.ajax({
