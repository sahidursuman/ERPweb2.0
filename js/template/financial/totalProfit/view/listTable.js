/*TMODJS:{"debug":true,"version":46,"md5":"96079c5bd060f3960fd027e22fbf04d8"}*/
    ...
    return require("../../../template")("financial/totalProfit/view/listTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(result, function(rs) {
                ...
                $out += "</td> <td>", $line = 19, null == rs.tripIncome || "" === rs.tripIncome ? ($out += "-", 
                $line = 19) : ($out += '<span class="F-float F-money">', $line = 19, $out += $escape(rs.tripIncome), 
                $out += "</span>", $line = 19), $out += "</td> <td>", $line = 20, null == rs.otherIncome || "" === rs.otherIncome ? ($out += "-", 
                $line = 20) : ($out += '<span class="F-float F-money">', $line = 20, $out += $escape(rs.otherIncome), 
                ...
                $out += "</span>", $line = 21), $out += "</td> <td>", $line = 22, null == rs.outCost || "" === rs.outCost ? ($out += "-", 
                $line = 22) : ($out += '<span class="F-float F-money">', $line = 22, $out += $escape(rs.outCost), 
                $out += "</span>", $line = 22), $out += "</td> <td>", $line = 23, null == rs.tripCost || "" === rs.tripCost ? ($out += "-", 
                $line = 23) : ($out += '<span class="F-float F-money">', $line = 23, $out += $escape(rs.tripCost), 
                $out += "</span>", $line = 23), $out += "</td> <td>", $line = 24, null == rs.transferCost || "" === rs.transferCost ? ($out += "-", 
                $line = 24) : ($out += '<span class="F-float F-money">', $line = 24, $out += $escape(rs.transferCost), 
                $out += "</span>", $line = 24), $out += "</td> <td>", $line = 25, null == rs.orderCost || "" === rs.orderCost ? ($out += "-", 
                $line = 25) : ($out += '<span class="F-float F-money">', $line = 25, $out += $escape(rs.orderCost), 
                $out += "</span>", $line = 25), $out += "</td> <td>", $line = 26, null == rs.income || "" === rs.income ? ($out += "-", 
                $line = 26) : ($out += '<span class="F-float F-money">', $line = 26, $out += $escape(rs.income), 
                $out += "</span>", $line = 26), $out += "</td> <td>", $line = 27, null == rs.cost || "" === rs.cost ? ($out += "-", 
                $line = 27) : ($out += '<span class="F-float F-money">', $line = 27, $out += $escape(rs.cost), 
