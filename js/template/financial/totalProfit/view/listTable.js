/*TMODJS:{"debug":true,"version":28,"md5":"a3a728c3f88652aa610bd7d08bc60845"}*/
    ...
    return require("../../../template")("financial/totalProfit/view/listTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(result, function(rs) {
                ...
                $out += "</span>", $line = 21), $out += "</td> <td>", $line = 22, null == rs.innerPay || "" == rs.innerPay ? ($out += "-", 
                $line = 22) : ($out += '<span class="F-float F-money">', $line = 22, $out += $escape(rs.innerPay), 
                $out += "</span>", $line = 22), $out += "</td> <td>", $line = 23, null == rs.transferPay || "" == rs.transferPay ? ($out += "-", 
                $line = 23) : ($out += '<span class="F-float F-money">', $line = 23, $out += $escape(rs.transferPay), 
                $out += "</span>", $line = 23), $out += "</td> <td>", $line = 24, null == rs.orderPay || "" == rs.orderPay ? ($out += "-", 
                $line = 24) : ($out += '<span class="F-float F-money">', $line = 24, $out += $escape(rs.orderPay), 
                $out += "</span>", $line = 24), $out += "</td> <td>", $line = 25, null == rs.incomeTrip || "" == rs.incomeTrip ? ($out += "-", 
                $line = 25) : ($out += '<span class="F-float F-money">', $line = 25, $out += $escape(rs.incomeTrip), 
                $out += "</span>", $line = 25), $out += "</td> <td>", $line = 26, null == rs.payTrip || "" == rs.payTrip ? ($out += "-", 
                $line = 26) : ($out += '<span class="F-float F-money">', $line = 26, $out += $escape(rs.payTrip), 
                $out += "</span>", $line = 26), $out += "</td> <td>", $line = 27, null == rs.profit || "" == rs.profit ? ($out += "-", 
                $line = 27) : ($out += '<span class="F-float F-money">', $line = 27, $out += $escape(rs.profit), 
                $out += "</span>", $line = 27), $out += "</td> </tr> ", $line = 29;
            }), new String($out);
        } catch (e) {
            throw {
                filename: $filename,
