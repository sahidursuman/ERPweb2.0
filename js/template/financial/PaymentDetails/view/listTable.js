/*TMODJS:{"debug":true,"version":35,"md5":"4aab77c2799349b924b36d31e18637e3"}*/
    ...
    return require("../../../template")("financial/PaymentDetails/view/listTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(result, function(rs) {
                $out += " <tr> <td>", $line = 3, $out += $escape(rs.accountTime), $out += "</td> <td>", 
                $line = 4, 0 == rs.moneyType ? ($out += "收入", $line = 4) : 1 == rs.moneyType && ($out += "支出", 
                ...
                0 == rs.moneyType ? ($out += ' <td class="F-float F-money">', $line = 11, $out += $escape(rs.incomeDifferenceMoney), 
                $out += "</td> <td>-</td> ", $line = 13) : ($out += ' <td>-</td> <td class="F-float F-money">', 
                $line = 15, $out += $escape(rs.payDifferenceMoney), $out += "</td> ", $line = 16), 
                $out += " <td>", $line = 17, null == rs.bankName || "" == rs.bankName ? ($out += "-", 
