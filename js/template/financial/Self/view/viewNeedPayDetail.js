/*TMODJS:{"debug":true,"version":9,"md5":"51589381c751a7ff4e155bae922980ca"}*/
define(function(require) {
    return require("../../../template")("financial/Self/view/viewNeedPayDetail", function($data, $filename) {
        try {
            ...
            $line = 25, $each(reconciliationDetailsList, function(rs) {
                ...
                $out += $escape(rs.businessType), $line = 28), $out += "</td> <td>", $line = 29, 
                $out += $escape(rs.price), $out += "</td> <td>", $line = 30, $out += $escape(rs.count), 
                $out += "</td> <td>", $line = 31, $out += $escape(rs.reduceMoney), $out += "</td> <td>", 
                $line = 32, $out += $escape(rs.needPayMoney), $out += "</td> <td>", $line = 33, 
                $out += $escape(rs.settlementMoney), $out += "</td> <td>", $line = 34, $out += $escape(rs.remark), 
                $out += "</td> <td>", $line = 35, null == rs.checkRealname || "" == rs.checkRealname ? ($out += "-", 
