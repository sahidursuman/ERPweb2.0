/*TMODJS:{"debug":true,"version":76,"md5":"e654e31dc286c15980989a00046457c4"}*/
    ...
    return require("../../../template")("financial/replace/view/replacePayingTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(bookinAccountList, function(rs) {
                ...
                $line = 7, $out += $string(rs.newDetail), $out += '" ><span style="height:35px;">', 
                $line = 7, $out += $string(rs.newDetail), $out += "</span></td> <td>", $line = 8, 
                $out += $escape(rs.creatorName), $out += "<br/>", $line = 8, $out += $escape($helpers.dateFormat(rs.creatorTime, "yyyy-MM-dd")), 
                $out += "<br/>", $line = 8, $out += $escape($helpers.dateFormat(rs.creatorTime, "hh:mm:ss")), 
                $out += '</td> <td><span class="F-float F-money">', $line = 9, $out += $escape(rs.bookingMoney), 
                $out += '</span></td> <td><a class="cursor T-action T-receive-money F-float F-money">', 
                ...
                $line = 14, $out += $escape(rs.payRemark), $out += "</textarea></td> <td>", $line = 15, 
