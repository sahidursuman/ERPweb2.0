/*TMODJS:{"debug":true,"version":90,"md5":"4152462a06e71e47403a40e9dacf4d52"}*/
    ...
    return require("../../../template")("financial/replace/view/replacePayingTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(bookinAccountList, function(rs) {
                $out += ' <tr data-id="', $line = 2, $out += $escape(rs.id), $out += '" data-arrid="', 
                $line = 2, $out += $escape(rs.arrangeId), $out += '"> <td><a href="javascript:void(0)" class="T-action T-viewDetail">', 
                $line = 3, $out += $escape(rs.orderNumber), $out += "</a></td> <td>", $line = 4, 
                $out += $escape(rs.startTime), $out += "</td> <td>", $line = 5, $out += $escape(rs.partnerAgencyName), 
                $out += "</td> <td>", $line = 6, $out += $escape(rs.projectName), $out += "</td> <td>", 
                $line = 7, $out += $escape(rs.projectList), $out += "</td> <td>", $line = 8, $out += $string(rs.newDetail), 
                $out += "</td> <td>", $line = 9, $out += $escape(rs.creatorName), $out += "<br/>", 
