/*TMODJS:{"debug":true,"version":43,"md5":"fafc7c023a5e16e4e6b6f52c2fedee17"}*/
    ...
    return require("../../../template")("financial/replace/view/replacePayingTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(bookinAccountList, function(rs) {
                ...
                $out += '"></td> <td><textarea class="col-sm-12 hct-textarea T-remark" name="payRemark" maxlength="1000" ', 
                $line = 12, !source && rs.unReceiveMoney <= 0 && ($out += " disabled ", $line = 12), 
                $out += "></textarea></td> <td>", $line = 13, $out += $escape(rs.checkTime), $out += "</td> <td>", 
                $line = 14, $out += $escape(rs.checkUserName), $out += '</td> <td> <label class="pos-rel"> <span class="lbl">', 
                $line = 17, rs.isConfirmAccount ? ($out += "已", $line = 17) : ($out += "未", $line = 17), 
                $out += '对账</span> </label> <label class="cursor" data-right="1290004"> <a> |</a></label> <a class="cursor T-action T-view-Received" data-right="1290004">查看</a> </td> </tr> ', 
                $line = 23;
            }), new String($out);
        } catch (e) {
            throw {
