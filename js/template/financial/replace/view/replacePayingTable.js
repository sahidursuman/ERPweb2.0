/*TMODJS:{"debug":true,"version":79,"md5":"f37b06eb1955c5b11f6ecb48de193fe7"}*/
    ...
    return require("../../../template")("financial/replace/view/replacePayingTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(bookinAccountList, function(rs) {
                ...
                $line = 6, $out += $escape(rs.projectName), $out += "</td> <td>", $line = 7, $out += $escape(rs.projectList), 
                $out += "</td> <td>", $line = 8, $out += $string(rs.newDetail), $out += "</td> <td>", 
                $line = 9, $out += $escape(rs.creatorName), $out += "<br/>", $line = 9, $out += $escape($helpers.dateFormat(rs.creatorTime, "yyyy-MM-dd")), 
                $out += "<br/>", $line = 9, $out += $escape($helpers.dateFormat(rs.creatorTime, "hh:mm:ss")), 
                $out += '</td> <td><span class="F-float F-money">', $line = 10, $out += $escape(rs.bookingMoney), 
