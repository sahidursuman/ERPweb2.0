/*TMODJS:{"debug":true,"version":17,"md5":"688820b128b6cf5f46fca19995e1d6a9"}*/
    ...
    return require("../../../../../template")("js/template/financial/transfer/view/viewPayedDetail", function($data, $filename) {
        try {
            ...
            $line = 23, $each(payedMoneyDetailList, function(rs) {
                ...
                $out += "</td> <td>", $line = 29, 0 == rs.payType ? ($out += " 现金 ", $line = 31) : 1 == rs.payType ? ($out += " 银行转账 ", 
                $line = 33) : 2 == rs.payType ? ($out += " 网上支付 ", $line = 35) : 3 == rs.payType ? ($out += " 支票 ", 
                $line = 37) : 4 == rs.payType && ($out += " 其他 ", $line = 39), $out += " </td> <td>", 
                $line = 41, $out += $escape(rs.remark), $out += "</td> <td>", $line = 42, null == rs.checkRealname || "" == rs.checkRealname ? ($out += "-", 
                $line = 42) : ($line = 42, $out += $escape(rs.checkRealname), $line = 42), $out += "</td> <td>", 
                $line = 43, null == rs.checkTime || "" == rs.checkTime ? ($out += "-", $line = 43) : ($line = 43, 
                $out += $escape($helpers.dateFormat(rs.checkTime, "yyyy-MM-dd hh:mm:ss")), $line = 43), 
                $out += "</td> </tr> ", $line = 45;
