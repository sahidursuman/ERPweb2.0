/*TMODJS:{"debug":true,"version":51,"md5":"09c11fc92255c8545eff25769bcc4977"}*/
    ...
    return require("../../../template")("financial/ticket/view/ticketPayingTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(financialTicketList, function(rs) {
                ...
                $out += '</td> <td class="T-payedDetail align-left" data-money="', $line = 12, $out += $escape(rs.payedMoney + rs.realGuidePayMoney), 
                $out += '"> <a class="T-action T-payDetails"> <span class="in-block">社付<span class="F-float F-money">', 
                $line = 14, $out += $escape(rs.payedMoney), $out += "</span></span> ", $line = 15, 
                1 != rs.isGuidePay && ($out += '<span class="in-block">导付<span class="F-float F-money" >', 
                $line = 15, $out += $escape(rs.realGuidePayMoney), $out += "</span></span>", $line = 15), 
                $out += " </a> </td> <td>", $line = 18, rs.billImage ? ($out += '<a class="cursor T-action T-view-receipts" data-billImage="', 
                $line = 18, $out += $escape(rs.billImage), $out += '">查看</a>', $line = 18) : ($out += '<span style="color:#bbb">查看</span>', 
                $line = 18), $out += '</td> <td class="F-float F-money">', $line = 19, $out += $escape(rs.settlementMoney), 
                $out += '</td> <td class="F-float F-money">', $line = 20, $out += $escape(rs.unPayedMoney), 
                $out += '</td> <td><input type="text" class="col-sm-12 T-reciveMoney F-float F-money" name="payMoney" value="', 
                $line = 21, $out += $escape(rs.payMoney), $out += '"></td> <td><textarea class="col-sm-12 hct-textarea T-payRemark" name="payRemark" maxlength="1000">', 
                $line = 22, $out += $escape(rs.payRemark), $out += "</textarea></td> <td>", $line = 23, 
                $out += $escape(rs.checkTime), $out += "</td> <td>", $line = 24, $out += $escape(rs.checkUserName), 
                $out += '</td> <td> <label class="pos-rel"> <span class="lbl">', $line = 27, rs.isConfirmAccount ? ($out += "已", 
                $line = 27) : ($out += "未", $line = 27), $out += '对账</span> </label> <label class="cursor R-right" data-right="1210004"> <a> |</a></label> <a class="cursor T-action T-view-details R-right" data-right="1210004">查看</a> </td> </tr> ', 
                $line = 33;
            }), new String($out);
        } catch (e) {
            throw {
