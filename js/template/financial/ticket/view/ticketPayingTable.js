/*TMODJS:{"debug":true,"version":12,"md5":"98b243e0ccc4365465fda3fe97dc7596"}*/
    ...
    return require("../../../template")("financial/ticket/view/ticketPayingTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(financialTicketList, function(rs, index) {
                ...
                $out += "</td> <td>", $line = 12, $out += $escape(rs.needPayMoney), $out += '</td> <td><a class="cursor T-action T-payDetails">社付', 
                $line = 13, $out += $escape(rs.payedMoney), $out += "，导付", $line = 13, $out += $escape(rs.realGuidePayMoney), 
                $out += '</a></td> <td><a class="cursor T-action T-view-receipts">查看</a></td> <td>', 
                $line = 15, $out += $escape(rs.settlementMoney), $out += "</td> <td>", $line = 16, 
                $out += $escape(rs.unPayedMoney), $out += '</td> <td><input type="text" class="col-sm-12 T-reciveMoney" name="payMoney" value="', 
                ...
                $line = 23) : ($out += "未", $line = 23), $out += '对账</span> </label> <label class="cursor"> <a> |</a></label> <a class="cursor T-action T-view-details">查看</a> </td> </tr> ', 
                $line = 29;
            }), new String($out);
        } catch (e) {
            throw {
