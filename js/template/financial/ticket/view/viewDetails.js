/*TMODJS:{"debug":true,"version":16,"md5":"5aa889d44a951cb429ef723c2bfe3521"}*/
    ...
    return require("../../../template")("financial/ticket/view/viewDetails", function($data, $filename) {
        try {
            ...
            $line = 18, $each(needPayDetailList, function(rs) {
                ...
                $line = 21, $out += $escape(rs.businessType), $out += "</td> <td>", $line = 22, 
                $out += $escape(rs.price), $out += "</td> <td>", $line = 23, $out += $escape(rs.count), 
                $out += "</td> <td>", $line = 24, $out += $escape(rs.reduceMoney), $out += "</td> <td>", 
                $line = 25, $out += $escape(rs.needPayMoney), $out += "</td> <td>", $line = 26, 
                $out += $escape(rs.settlementMoney), $out += "</td> <td>", $line = 27, $out += $escape(rs.remark), 
                $out += "</td> <td>", $line = 28, $out += $escape(rs.checkRealname), $out += "</td> <td>", 
                $line = 29, $out += $escape(rs.checkTime), $out += "</td> </tr> ", $line = 31;
