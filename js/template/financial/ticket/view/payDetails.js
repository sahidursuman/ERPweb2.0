/*TMODJS:{"debug":true,"version":8,"md5":"1155f0cf558da58b2a2550e97ed2e66b"}*/
    ...
    return require("../../../template")("financial/ticket/view/payDetails", function($data, $filename) {
        try {
            ...
            $line = 16, $each(payedMoneyDetailList, function(item) {
                ...
                $out += $escape(item.payParty), $out += '</td> <td class="F-float F-money">', $line = 21, 
                $out += $escape(item.payDifferenceMoney), $out += "</td> <td>", $line = 22, 0 == item.payType ? ($out += "现金", 
                $line = 22) : 1 == item.payType ? ($out += "银行转账", $line = 22) : 2 == item.payType ? ($out += "网上支付", 
                $line = 22) : 3 == item.payType ? ($out += "支票", $line = 22) : ($out += "其它", $line = 22), 
