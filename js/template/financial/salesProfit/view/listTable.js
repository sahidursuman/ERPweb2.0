/*TMODJS:{"debug":true,"version":58,"md5":"366fd02ac96128a273f799fddc1c4c40"}*/
    ...
    return require("../../../template")("financial/salesProfit/view/listTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(result, function(rs) {
                ...
                $line = 2, $out += $escape(rs.lineProductId), $out += '" data-type="', $line = 2, 
                $out += $escape(rs.type), $out += '"> <td><a href="javascript:void(0)" class="T-viewTouristOrBooking">', 
                $line = 3, null == rs.orderNumber || "" == rs.orderNumber ? ($out += "-", $line = 3) : ($line = 3, 
                $out += $escape(rs.orderNumber), $line = 3), $out += "</a></td> <td>", $line = 4, 
                null == rs.lineProductName || "" == rs.lineProductName ? ($out += "-", $line = 4) : ($line = 4, 
                $out += $escape(rs.lineProductName), $line = 4), $out += "</td> <td>", $line = 5, 
                null == rs.startTime || "" == rs.startTime ? ($out += "-", $line = 5) : ($line = 5, 
                ...
                $line = 17), $out += "</td> <td>", $line = 18, rs.currentMoneyProfit || "0" == rs.currentMoneyProfit ? ($out += '<span class="F-float F-money">', 
                $line = 18, $out += $escape(rs.currentMoneyProfit), $out += "</span>", $line = 18) : ($out += "-", 
                $line = 18), $out += "</td> <td>", $line = 19, rs.orderIncome || "0" == rs.orderIncome ? ($out += '<span class="F-float F-money">', 
                $line = 19, $out += $escape(rs.orderIncome), $out += "</span>", $line = 19) : ($out += "-", 
                $line = 19), $out += "</td> <td>", $line = 20, rs.outCost || "0" == rs.outCost ? ($out += '<span class="F-float F-money">', 
                $line = 20, $out += $escape(rs.outCost), $out += "</span>", $line = 20) : ($out += "-", 
                $line = 20), $out += "</td> <td>", $line = 21, rs.tripCost || "0" == rs.tripCost ? ($out += '<span class="F-float F-money">', 
                $line = 21, $out += $escape(rs.tripCost), $out += "</span>", $line = 21) : ($out += "-", 
                $line = 21), $out += "</td> <td>", $line = 22, rs.transferCost || "0" == rs.transferCost ? ($out += '<span class="F-float F-money">', 
                $line = 22, $out += $escape(rs.transferCost), $out += "</span>", $line = 22) : ($out += "-", 
                $line = 22), $out += "</td> <td>", $line = 23, rs.innerCost || "0" == rs.innerCost ? ($out += '<span class="F-float F-money">', 
                $line = 23, $out += $escape(rs.innerCost), $out += "</span>", $line = 23) : ($out += "-", 
                $line = 23), $out += "</td> <td>", $line = 24, rs.orderCost || "0" == rs.orderCost ? ($out += '<span class="F-float F-money">', 
                $line = 24, $out += $escape(rs.orderCost), $out += "</span>", $line = 24) : ($out += "-", 
                $line = 24), $out += "</td> <td>", $line = 25, rs.income || "0" == rs.income ? ($out += '<span class="F-float F-money">', 
                $line = 25, $out += $escape(rs.income), $out += "</span>", $line = 25) : ($out += "-", 
                $line = 25), $out += "</td> <td>", $line = 26, rs.cost || "0" == rs.cost ? ($out += '<span class="F-float F-money">', 
                $line = 26, $out += $escape(rs.cost), $out += "</span>", $line = 26) : ($out += "-", 
                $line = 26), $out += "</td> <td>", $line = 27, rs.profit || "0" == rs.profit ? ($out += '<span class="F-float F-money">', 
                $line = 27, $out += $escape(rs.profit), $out += "</span>", $line = 27) : ($out += "-", 
                $line = 27), $out += "</td> <td>", $line = 28, rs.avgProfit || "0" == rs.avgProfit ? ($out += '<span class="F-float F-money">', 
                $line = 28, $out += $escape(rs.avgProfit), $out += "</span>", $line = 28) : ($out += "-", 
                $line = 28), $out += "</td> </tr> ", $line = 30;
            }), new String($out);
        } catch (e) {
            throw {
                filename: $filename,
