/*TMODJS:{"debug":true,"version":82,"md5":"45071b5d05f0c130dbdeede819020830"}*/
    ...
    return require("../../../template")("financial/guide/view/guideCheckingTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(list, function(guide) {
                ...
                $line = 30, $out += $escape(guide.settlementMoney), $out += '" ', $line = 30, guide.isConfirmAccount && ($out += ' disabled="disabled"', 
                $line = 30), $out += '> </td> <td class="T-payedDetail F-float F-money" data-money="', 
                $line = 31, $out += $escape(guide.payedMoney), $out += '"><a href="javascript:void(0);" class="T-action T-viewPayedMoney">', 
                $line = 31, $out += $escape(guide.payedMoney), $out += '</a></td> <td name="unPayedMoney"><span class="F-float F-money">', 
                $line = 32, $out += $escape(guide.unPayedMoney), $out += '</span></td> <td><textarea class="col-sm-12 hct-textarea T-checkRemark" name="checkRemark" maxlength="1000" ', 
                $line = 33, guide.isConfirmAccount && ($out += ' disabled="disabled"', $line = 33), 
                $out += ">", $line = 33, $out += $escape(guide.checkRemark), $out += "</textarea></td> <td>", 
                $line = 34, $out += $escape(guide.checkTime), $out += "</td> <td>", $line = 35, 
                $out += $escape(guide.checkRealName), $out += '</td> <td> <label class="pos-rel R-right" ', 
                $line = 37, guide.isConfirmAccount ? ($out += ' data-right="1200005" ', $line = 37) : ($out += ' data-right="1200002" ', 
                $line = 37), $out += '> <input type="checkbox" class="ace T-checkbox" ', $line = 38, 
                (guide.change && 1 == guide.isChecked || !guide.change && 1 == guide.isConfirmAccount) && ($out += ' checked="checked" ', 
                $line = 38), $out += '/> <span class="lbl">对账</span> </label> <label class="cursor" ', 
                $line = 41, guide.isConfirmAccount ? ($out += ' data-right="1200005" ', $line = 41) : ($out += ' data-right="1200002" ', 
                $line = 41), $out += '> <a data-right="1200003"> |</a></label> <a class="cursor T-action T-view" data-right="1200003">查看</a> </td> </tr> ', 
                $line = 45;
            }), new String($out);
        } catch (e) {
            throw {
