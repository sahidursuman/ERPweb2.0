/*TMODJS:{"debug":true,"version":12,"md5":"6e860892c9b239bcf84f5e506de5d5ad"}*/
    ...
    return require("../../../template")("financial/guide/view/operationDetail", function($data, $filename) {
        try {
            ...
            $line = 26, $each(payedRecordList, function(item) {
                ...
                $out += $escape(item.payParty), $out += "</td> <td>", $line = 31, $out += $escape(item.settlementMoney), 
                $out += "</td> <td>", $line = 32, $out += $escape($helpers.getPayTypeText(item.payType)), 
                $out += "</td> <td>", $line = 33, $out += $escape(item.remark), $out += "</td> <td>", 
                $line = 34, $out += $escape(item.checkRealname), $out += "</td> <td>", $line = 35, 
                $out += $escape(item.checkTime), $out += "</td> </tr> ", $line = 37;
                ...
                $out += $escape(item.settlementMoney), $out += "</td> <td>", $line = 44, $out += $escape(item.remark), 
