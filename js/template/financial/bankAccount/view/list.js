/*TMODJS:{"debug":true,"version":131,"md5":"830cd6a10231233d73948912b1aee684"}*/
    ...
    return require("../../../template")("financial/bankAccount/view/list", function($data, $filename) {
        try {
            ...
            $line = 42, $each(newBankAccountList, function(rs) {
                $out += ' <tr bankid="', $line = 43, $out += $escape(rs.id), $out += '" aliasName="', 
                $line = 43, $out += $escape(rs.aliasName), $out += '" bankMoney="', $line = 43, 
                $out += $escape(rs.balance), $out += '" beginningBalance="', $line = 43, $out += $escape(rs.beginningBalance), 
                $out += '" data-type="', $line = 43, $out += $escape(rs.type), $out += '"> <td>', 
                $line = 44, 0 == rs.type ? ($out += "现金账户", $line = 44) : ($out += "银行账户", $line = 44), 
                $out += "</td> <td>", $line = 45, $out += $escape(rs.aliasName), $out += "</td> <td>", 
                $line = 46, 0 == rs.type ? ($out += "-", $line = 46) : ($line = 46, $out += $escape(rs.bankAccountNumber), 
                $line = 46), $out += "</td> <td>", $line = 47, 0 == rs.type ? ($out += "-", $line = 47) : ($line = 47, 
                $out += $escape(rs.openingBank), $line = 47), $out += "</td> <td>", $line = 48, 
                0 == rs.type ? ($out += "-", $line = 48) : ($line = 48, $out += $escape(rs.accountName), 
                $line = 48), $out += "</td> <td>", $line = 49, $out += $escape($helpers.dateFormat(rs.beginningTime, "yyyy-MM-dd")), 
                $out += '</td> <td class="F-float F-money">', $line = 50, $out += $escape(rs.beginningBalance), 
                $out += '</td> <td class="F-float F-money">', $line = 51, $out += $escape(rs.incomeMoney), 
                $out += '</td> <td class="F-float F-money">', $line = 52, $out += $escape(rs.payMoney), 
                $out += "</td> <td>", $line = 53, $out += $escape(rs.balance), $out += '</td> <td> <div class="btn-group"> <a class="T-action T-view cursor"> 查看 </a> </div> </td> </tr> ', 
                $line = 62;
