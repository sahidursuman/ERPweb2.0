/*TMODJS:{"debug":true,"version":69,"md5":"5447451f593242e404a70829ce217e5a"}*/
define(function(require) {
    return require("../../../template")("financial/bankAccount/view/list", function($data, $filename) {
        try {
            var $utils = this, $line = ($utils.$helpers, 0), $escape = $utils.$escape, beginningBalance = $data.beginningBalance, incomeMoney = $data.incomeMoney, payMoney = $data.payMoney, $each = $utils.$each, newBankAccountList = $data.newBankAccountList, $out = ($data.rs, 
            $data.$index, "");
            ...
            $line = 34, $each(newBankAccountList, function(rs) {
                $out += ' <tr bankid="', $line = 35, $out += $escape(rs.id), $out += '" > <td>', 
                $line = 36, $out += $escape(rs.bankAccountNumber), $out += "</td> <td> ", $line = 38, 
                $out += $escape(rs.openingBank), $out += " </td> <td>", $line = 40, $out += $escape(rs.accountName), 
                $out += "</td> <td>", $line = 41, $out += $escape(rs.beginningTime), $out += '</td> <td class="">', 
                $line = 42, $out += $escape(rs.beginningBalance), $out += "</td> <td>", $line = 43, 
                $out += $escape(rs.incomeMoney), $out += "</td> <td>", $line = 44, $out += $escape(rs.payMoney), 
