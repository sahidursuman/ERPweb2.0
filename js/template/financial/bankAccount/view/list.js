/*TMODJS:{"debug":true,"version":129,"md5":"66cefba010618e9deb6d37c62c4a88e7"}*/
    ...
    return require("../../../template")("financial/bankAccount/view/list", function($data, $filename) {
        try {
            ...
            $line = 39, $each(newBankAccountList, function(rs) {
                $out += ' <tr bankid="', $line = 40, $out += $escape(rs.id), $out += '" aliasName="', 
                $line = 40, $out += $escape(rs.aliasName), $out += '" bankMoney="', $line = 40, 
                $out += $escape(rs.balance), $out += '" beginningBalance="', $line = 40, $out += $escape(rs.beginningBalance), 
                $out += '" data-type="', $line = 40, $out += $escape(rs.type), $out += '"> <td>', 
                $line = 41, 0 == rs.type ? ($out += "现金账户", $line = 41) : ($out += "银行账户", $line = 41), 
                $out += "</td> <td>", $line = 42, $out += $escape(rs.aliasName), $out += "</td> <td>", 
                $line = 43, 0 == rs.type ? ($out += "-", $line = 43) : ($line = 43, $out += $escape(rs.bankAccountNumber), 
                $line = 43), $out += "</td> <td>", $line = 44, 0 == rs.type ? ($out += "-", $line = 44) : ($line = 44, 
                $out += $escape(rs.openingBank), $line = 44), $out += "</td> <td>", $line = 45, 
                0 == rs.type ? ($out += "-", $line = 45) : ($line = 45, $out += $escape(rs.accountName), 
                $line = 45), $out += "</td> <td>", $line = 46, $out += $escape($helpers.dateFormat(rs.beginningTime, "yyyy-MM-dd")), 
                ...
                $out += "</td> <td>", $line = 50, $out += $escape(rs.balance), $out += '</td> <td> <div class="btn-group"> <a class="T-action T-view cursor"> 查看 </a> </div> </td> </tr> ', 
                $line = 59;
