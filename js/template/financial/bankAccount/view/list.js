/*TMODJS:{"debug":true,"version":132,"md5":"8b60950596c9ab2514032731c3d154fb"}*/
    ...
    return require("../../../template")("financial/bankAccount/view/list", function($data, $filename) {
        try {
            ...
            $line = 42, $each(newBankAccountList, function(rs) {
                $out += ' <tr bankid="', $line = 43, $out += $escape(rs.id), $out += '" banknum="', 
                $line = 43, $out += $escape(rs.bankAccountNumber), $out += '" bankMoney="', $line = 43, 
                $out += $escape(rs.balance), $out += '" data-type="', $line = 43, $out += $escape(rs.type), 
                $out += '"> <td>', $line = 44, 0 == rs.type ? ($out += "现金账户", $line = 44) : ($out += "银行账户", 
                $line = 44), $out += "</td> <td>", $line = 45, $out += $escape(rs.aliasName), $out += "</td> <td>", 
