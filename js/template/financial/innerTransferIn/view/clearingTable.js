/*TMODJS:{"debug":true,"version":29,"md5":"f8c0a122aaebccfc437c9046c469c051"}*/
    ...
    return require("../../../template")("financial/innerTransferIn/view/clearingTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(innerTransferIncomeDetailsList, function(checking) {
                ...
                $line = 28, $out += $escape(checking.payMoney), $out += '" name="payMoney" value="', 
                $line = 28, $out += $escape(checking.payMoney), $out += '" style="text-align:center"/></td> <td><textarea class="col-sm-12 hct-textarea T-remark" maxlength="1000" name="payRemark">', 
                $line = 29, $out += $escape(checking.payRemark), $out += "</textarea></td> <td>", 
                $line = 30, null == checking.checkTime || "" == checking.checkTime ? ($out += "-", 
                $line = 30) : ($line = 30, $out += $escape($helpers.dateFormat(checking.checkTime, "yyyy-MM-dd hh:mm:ss")), 
                $line = 30), $out += "</td> <td>", $line = 31, null == checking.checkUserName || "" == checking.checkUserName ? ($out += "-", 
                $line = 31) : ($line = 31, $out += $escape(checking.checkUserName), $line = 31), 
                $out += '</td> <td><label class="pos-rel"> ', $line = 33, 1 == checking.isConfirmAccount ? ($out += " 已对账 ", 
                $line = 35) : ($out += " 未对账 ", $line = 37), $out += ' </label> <label> <a href="#" class="T-action T-needPayDetail" style="margin-left:5px">查看</a> </label> </td> </tr> ', 
                $line = 44;
            }), new String($out);
        } catch (e) {
            throw {
