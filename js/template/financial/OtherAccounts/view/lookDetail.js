/*TMODJS:{"debug":true,"version":57,"md5":"4e48f7a412f68bcb6617a450cb38eac9"}*/
    ...
    return require("../../../template")("financial/OtherAccounts/view/lookDetail", function($data, $filename) {
        try {
            ...
            $line = 13, $each(paymentDetailsList, function(item) {
                ...
                $out += "</td> <td>", $line = 19, $out += $escape($helpers.getPayTypeText(item.payType)), 
                $out += "</td> <td>", $line = 20, $out += $escape(item.remark), $out += "</td> <td>", 
                $line = 21, $out += $escape(item.checkRealname), $out += "</td> <td>", $line = 22, 
                $out += $escape(item.checkTime), $out += "</td> </tr> ", $line = 24;
