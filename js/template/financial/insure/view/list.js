/*TMODJS:{"debug":true,"version":406,"md5":"8dc77db58d7e6bfac46e249c07248b1d"}*/
define(function(require) {
    return require("../../../template")("financial/insure/view/list", function($data, $filename) {
        try {
            var $utils = this, $helpers = $utils.$helpers, $line = 0, searchParam = $data.searchParam, $escape = $utils.$escape, $each = $utils.$each, financialInsuranceList = $data.financialInsuranceList, $out = ($data.rs, 
            $data.$index, "");
            ...
            $line = 34, $each(financialInsuranceList, function(rs) {
                ...
                $line = 36) : ($line = 36, $out += $escape(rs.insuranceName), $line = 36), $out += '</td> <td class="F-float F-money">', 
                $line = 37, $out += $escape(rs.sumNeedPayMoney), $out += '</td> <td class="F-float F-money">', 
                $line = 38, $out += $escape(rs.sumSettlementMoney), $out += '</td> <td class="F-float F-money">', 
                $line = 39, $out += $escape(rs.sumPayedMoney), $out += '</td> <td class="F-float F-money"><span ', 
                $line = 40, 0 != rs.sumUnPayedMoney && ($out += 'style="color:red"', $line = 40), 
                $out += ">", $line = 40, $out += $escape(rs.sumUnPayedMoney), $out += "</span></td> <td>", 
