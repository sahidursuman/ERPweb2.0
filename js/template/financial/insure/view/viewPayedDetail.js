/*TMODJS:{"debug":true,"version":7,"md5":"a35d4420c9360ecb6e364c3837e9dd3c"}*/
define(function(require) {
    return require("../../../template")("financial/insure/view/viewPayedDetail", function($data, $filename) {
        try {
            ...
            $line = 24, $each(payedMoneyDetailList, function(rs) {
                ...
                $line = 28, null == rs.payParty || "" == rs.payParty ? ($out += "-", $line = 28) : ($line = 28, 
                $out += $escape(rs.payParty), $line = 28), $out += '</td> <td class="F-float F-money">', 
                $line = 29, $out += $escape(rs.payDifferenceMoney), $out += '</td> <td class="F-float F-money">', 
                $line = 30, $out += $escape($helpers.getPayTypeText(rs.payType)), $out += "</td> <td>", 
                $line = 31, null == rs.accountName || "" == rs.accountName ? ($out += "-", $line = 31) : ($line = 31, 
                $out += $escape(rs.accountName), $line = 31), $out += "</td> <td>", $line = 32, 
                $out += $escape(rs.remark), $out += "</td> <td>", $line = 33, null == rs.checkRealname || "" == rs.checkRealname ? ($out += "-", 
                $line = 33) : ($line = 33, $out += $escape(rs.checkRealname), $line = 33), $out += "</td> <td>", 
                $line = 34, null == rs.checkTime || "" == rs.checkTime ? ($out += "-", $line = 34) : ($line = 34, 
                $out += $escape($helpers.dateTimeFormat(rs.checkTime)), $line = 34), $out += "</td> </tr> ", 
                $line = 36;
