/*TMODJS:{"debug":true,"version":3,"md5":"1a073b3664628c6cbfc6cf195fe5004a"}*/
    ...
    return require("../../../template")("financial/insure/view/viewPayedDetail", function($data, $filename) {
        try {
            ...
            $line = 23, $each(payedMoneyDetailList, function(rs) {
                ...
                $out += $escape(rs.businessType), $line = 26), $out += '</td> <td class="F-float F-money">', 
                $line = 27, null == rs.payParty || "" == rs.payParty ? ($out += "-", $line = 27) : ($line = 27, 
                $out += $escape(rs.payParty), $line = 27), $out += '</td> <td class="F-float F-money">', 
                $line = 28, $out += $escape(rs.payDifferenceMoney), $out += '</td> <td class="F-float F-money">', 
                $line = 29, $out += $escape($helpers.getPayTypeText(rs.payType)), $out += "</td> <td>", 
                $line = 30, $out += $escape(rs.remark), $out += "</td> <td>", $line = 31, null == rs.checkRealname || "" == rs.checkRealname ? ($out += "-", 
                $line = 31) : ($line = 31, $out += $escape(rs.checkRealname), $line = 31), $out += "</td> <td>", 
                $line = 32, null == rs.checkTime || "" == rs.checkTime ? ($out += "-", $line = 32) : ($line = 32, 
