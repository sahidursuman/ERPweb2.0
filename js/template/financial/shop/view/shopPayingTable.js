/*TMODJS:{"debug":true,"version":28,"md5":"68f980db654f4d2350ba4f104bef6565"}*/
    ...
    return require("../../../template")("financial/shop/view/shopPayingTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(shopAccountList, function(rs) {
                ...
                $out += '</span></td> <td style="white-space: nowrap;">', $line = 10, rs.billImage ? ($out += '<a class="cursor T-action T-view-receipts" data-billimage="', 
                $line = 10, $out += $escape(rs.billImage), $out += '">查看</a>', $line = 10) : ($out += '<span style="color:#bbb">查看</span>', 
                $line = 10), $out += '</td> <td><span class="F-float F-money">', $line = 11, $out += $escape(rs.backMoney), 
                $out += '</span></td> <td><a class="cursor T-action T-payDetails">&nbsp;&nbsp;社收:<span class="F-float F-money">', 
                $line = 12, $out += $escape(rs.receiveMoney), $out += '</span>&nbsp;&nbsp;导游现收:<span class="F-float F-money">', 
                $line = 12, $out += $escape(rs.rebateCurrentIncomeMoney), $out += '</span></a></td> <td><span class="F-float F-money">', 
                $line = 12, $out += $escape(rs.settlementMoney), $out += '</span></td> <td name="unPayedMoney"><span class="F-float F-money">', 
                $line = 13, $out += $escape(rs.unReceiveMoney), $out += '</span></td> <td><input type="text" class="col-sm-12 T-reciveMoney F-float F-money" name="payMoney" data-le="', 
                $line = 14, $out += $escape(rs.unReceiveMoney), $out += '" ', $line = 14, !source && rs.unReceiveMoney <= 0 && ($out += " disabled ", 
                $line = 14), $out += ' value="', $line = 14, $out += $escape(rs.payMoney), $out += '"></td> <td><input type="text" class="col-sm-12 T-payRemark" name="payRemark" ', 
                $line = 15, !source && rs.unReceiveMoney <= 0 && ($out += " disabled ", $line = 15), 
                $out += ' value="', $line = 15, $out += $escape(rs.IncomeRemark), $out += '"></td> <td>', 
                $line = 16, $out += $escape(rs.checkTime), $out += "</td> <td>", $line = 17, $out += $escape(rs.checkName), 
                $out += '</td> <td> <label class="pos-rel"> <span class="lbl">', $line = 20, rs.isConfirmAccount ? ($out += "已", 
                $line = 20) : ($out += "未", $line = 20), $out += '对账</span> </label> <label class="cursor" data-right="1310004"> <a> |</a></label> <a class="cursor T-action T-view-details" data-right="1310004">查看</a> </td> </tr> ', 
                $line = 26, rs.shopItem.length > 0 && ($out += ' <tr class="hide"> <td colspan="19"> <table class="table table-striped table-bordered table-hover"> <thead> <tr> <th class="th-border" style="min-width: 45px;">序号</th> <th class="th-border" style="min-width: 70px;">商品</th> <th class="th-border" style="min-width: 120px;">消费金额</th> <th class="th-border" style="min-width: 45px;">社佣</th> <th class="th-border" style="min-width: 45px;">导佣</th> </tr> </thead> <tbody> ', 
                $line = 40, $each(rs.shopItem, function(list, i) {
                    $out += " <tr> <td>", $line = 42, $out += $escape(i + 1), $out += "</td> <td>", 
                    $line = 43, $out += $escape(list.itemName), $out += '</td> <td><span class="F-float F-money">', 
                    $line = 44, $out += $escape(list.consumeMoney), $out += '</span></td> <td><span class="F-float F-money">', 
                    $line = 45, $out += $escape(list.travelAgencyRebateMoney), $out += '</span></td> <td><span class="F-float F-money">', 
                    $line = 46, $out += $escape(list.guideRebateMoney), $out += "</span></td> </tr> ", 
                    $line = 48;
                }), $out += " </tbody> </table> </td> </tr> ", $line = 54), $out += " ", $line = 55;
            }), new String($out);
        } catch (e) {
            throw {
                filename: $filename,
