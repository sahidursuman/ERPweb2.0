/*TMODJS:{"debug":true,"version":27,"md5":"abbd0181914275db85872e80510e45cb"}*/
    ...
    return require("../../../template")("financial/shop/view/shopPayingTable", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(shopAccountList, function(rs) {
                ...
                $line = 12, $out += $escape(rs.receiveMoney), $out += '</a> <td><span class="F-float F-money">', 
                $line = 13, $out += $escape(rs.settlementMoney), $out += '</span></td> <td name="unPayedMoney"><span class="F-float F-money">', 
                $line = 14, $out += $escape(rs.unReceiveMoney), $out += '</span></td> <td><input type="text" class="col-sm-12 T-reciveMoney F-float F-money" name="payMoney" data-le="', 
                $line = 15, $out += $escape(rs.unReceiveMoney), $out += '" ', $line = 15, !source && rs.unReceiveMoney <= 0 && ($out += " disabled ", 
                $line = 15), $out += ' value="', $line = 15, $out += $escape(rs.payMoney), $out += '"></td> <td><input type="text" class="col-sm-12 T-payRemark" name="payRemark" ', 
                $line = 16, !source && rs.unReceiveMoney <= 0 && ($out += " disabled ", $line = 16), 
                $out += ' value="', $line = 16, $out += $escape(rs.IncomeRemark), $out += '"></td> <td>', 
                $line = 17, $out += $escape(rs.checkTime), $out += "</td> <td>", $line = 18, $out += $escape(rs.checkName), 
                $out += '</td> <td> <label class="pos-rel"> <span class="lbl">', $line = 21, rs.isConfirmAccount ? ($out += "已", 
                $line = 21) : ($out += "未", $line = 21), $out += '对账</span> </label> <label class="cursor" data-right="1310004"> <a> |</a></label> <a class="cursor T-action T-view-details" data-right="1310004">查看</a> </td> </tr> ', 
                $line = 27, rs.shopItem.length > 0 && ($out += ' <tr class="hide"> <td colspan="19"> <table class="table table-striped table-bordered table-hover"> <thead> <tr> <th class="th-border" style="min-width: 45px;">序号</th> <th class="th-border" style="min-width: 70px;">商品</th> <th class="th-border" style="min-width: 120px;">消费金额</th> <th class="th-border" style="min-width: 45px;">社佣</th> <th class="th-border" style="min-width: 45px;">导佣</th> </tr> </thead> <tbody> ', 
                $line = 41, $each(rs.shopItem, function(list, i) {
                    $out += " <tr> <td>", $line = 43, $out += $escape(i + 1), $out += "</td> <td>", 
                    $line = 44, $out += $escape(list.itemName), $out += '</td> <td><span class="F-float F-money">', 
                    $line = 45, $out += $escape(list.consumeMoney), $out += '</span></td> <td><span class="F-float F-money">', 
                    $line = 46, $out += $escape(list.travelAgencyRebateMoney), $out += '</span></td> <td><span class="F-float F-money">', 
                    $line = 47, $out += $escape(list.guideRebateMoney), $out += "</span></td> </tr> ", 
                    $line = 49;
                }), $out += " </tbody> </table> </td> </tr> ", $line = 55), $out += " ", $line = 56;
            }), new String($out);
        } catch (e) {
            throw {
                filename: $filename,
