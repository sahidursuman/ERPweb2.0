/*TMODJS:{"debug":true,"version":119,"md5":"e91d28d2180aeba268d019df30e9561e"}*/
    ...
    return require("../../../template")("financial/count/view/listTable", function($data, $filename) {
        try {
            ...
            $line = 22, $each(tripPlanList, function(financialTripPlan) {
                ...
                $out += '" class=" btn-count-examine cursor"> 明细 </a> ', $line = 75, -1 == financialTripPlan.tripPlan.billStatus && ($out += ' <a data-entity-billStatus="', 
                $line = 76, $out += $escape(financialTripPlan.tripPlan.billStatus), $out += '" data-entity-id="', 
                $line = 76, $out += $escape(financialTripPlan.id), $out += '" class="cursor btn-guide-account R-right" data-right="1190004"> <label class="padding-right20">|</label> 报账 </a> ', 
                $line = 80), $out += " </td> </tr> ", $line = 83, financialTripPlan.tripPlan.tripPlanDetails.length > 1 && ($out += " ", 
                $line = 84, $each(financialTripPlan.tripPlan.tripPlanDetails, function(tripPlanDetails, index) {
                    $out += " ", $line = 85, index > 0 && ($out += " <tr> <td>", $line = 87, $out += $escape($helpers.dateFormat(tripPlanDetails.startTime, "yyyy-MM-dd")), 
                    $out += "</td> <td>", $line = 88, $out += $escape($helpers.dateFormat(tripPlanDetails.endTime, "yyyy-MM-dd")), 
                    $out += "</td> <td>", $line = 89, $out += $escape(tripPlanDetails.lineProductName), 
                    $out += "</td> <td>", $line = 90, $out += $escape(tripPlanDetails.sumMemberCount), 
                    $out += "</td> </tr> ", $line = 92), $out += " ", $line = 93;
                }), $out += " ", $line = 94), $out += " ", $line = 95), $out += " ", $line = 96;
            ...
            $line = 101, $out += $escape(recordSize), $out += ' 条记录</div> </div> <div class="col-xs-6"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div>', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
