/*TMODJS:{"debug":true,"version":129,"md5":"2b234ed4f261e146c2330e3be5dade91"}*/
define(function(require) {
    return require("../../../template")("financial/count/view/listTable", function($data, $filename) {
        try {
            ...
            $data.recordSize), $out = "";
            return $out += '<table class="table table-striped table-bordered table-hover T-NotShowHighLight" style="margin-top: 20px"> <thead> <tr class="bg-blur"> <th style="min-width:50px">团号</th> <th style="min-width:70px">出团日期</th> <th style="min-width:70px">完团日期</th> <th style="min-width:80px">线路产品</th> <th style="min-width:40px">人数</th> <th style="min-width:80px">团队人数</th> <th style="min-width:50px">导游</th> <th style="min-width:40px">车牌号</th> <th style="min-width:40pxpx">收入</th> <th style="min-width:40pxpx">成本</th> <th style="min-width:40pxpx">毛利</th> <th style="min-width:40px">人均毛利</th> <th style="min-width:80px">审核状态</th> <th style="min-width:40px">审核人</th> <th style="min-width:130px">操作</th> </tr> </thead> <tbody> ', 
            $line = 22, $each(tripPlanList, function(financialTripPlan) {
                ...
                $out += '"', $line = 65), $out += '> <a data-entity-billStatus="', $line = 66, $out += $escape(financialTripPlan.tripPlan.billStatus), 
                $out += '" data-entity-id="', $line = 66, $out += $escape(financialTripPlan.id), 
                $out += '" data-guideFinancialExamine="', $line = 66, $out += $escape(financialTripPlan.guideFinancialExamine), 
                $out += '" class="cursor btn-count-update T-audit"> 审核 <label style="padding-left:10px;">|</label> </a> <a data-entity-id="', 
                $line = 70, $out += $escape(financialTripPlan.id), $out += '" class=" btn-count-examine cursor"> 明细 </a> ', 
                $line = 73, -1 == financialTripPlan.tripPlan.billStatus && ($out += ' <a data-entity-billStatus="', 
                $line = 74, $out += $escape(financialTripPlan.tripPlan.billStatus), $out += '" data-entity-id="', 
                $line = 74, $out += $escape(financialTripPlan.id), $out += '" class="cursor btn-guide-account R-right" data-right="1190004"> <label class="padding-right20">|</label> 报账 </a> ', 
                $line = 78), $out += " ", $line = 79, financialTripPlan.tripPlan && ($out += ' <a class="cursor R-right T-quality" data-id="', 
                $line = 80, $out += $escape(financialTripPlan.tripPlan.id), $out += '" data-right="1190004"> <label class="padding-right20">|</label> 质量 </a> ', 
                $line = 84), $out += " </td> </tr> ", $line = 87, financialTripPlan.tripPlan.tripPlanDetails.length > 1 && ($out += " ", 
                $line = 88, $each(financialTripPlan.tripPlan.tripPlanDetails, function(tripPlanDetails, index) {
                    $out += " ", $line = 89, index > 0 && ($out += " <tr> <td>", $line = 91, $out += $escape($helpers.dateFormat(tripPlanDetails.startTime, "yyyy-MM-dd")), 
                    $out += "</td> <td>", $line = 92, $out += $escape($helpers.dateFormat(tripPlanDetails.endTime, "yyyy-MM-dd")), 
                    $out += "</td> <td>", $line = 93, $out += $escape(tripPlanDetails.lineProductName), 
                    $out += "</td> <td>", $line = 94, $out += $escape(tripPlanDetails.sumMemberCount), 
                    $out += "</td> </tr> ", $line = 96), $out += " ", $line = 97;
                }), $out += " ", $line = 98), $out += " ", $line = 99), $out += " ", $line = 100;
            ...
            $line = 105, $out += $escape(recordSize), $out += ' 条记录</div> </div> <div class="col-xs-6"> <div class="dataTables_paginate paging_simple_numbers T-pagenation"> </div> </div> </div>', 
            new String($out);
        } catch (e) {
            throw {
                filename: $filename,
