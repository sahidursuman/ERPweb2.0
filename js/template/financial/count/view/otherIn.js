/*TMODJS:{"debug":true,"version":48,"md5":"06debe990cbec6f439fdf87cbefc607e"}*/
    ...
    return require("../../../template")("financial/count/view/otherIn", function($data, $filename) {
        try {
            ...
            $each(arrangeIncomePaymentList, function(otherInCome) {
                ...
                $line = 31), $out += '/></td> <td><input style="width:90px;" type="text" class="F-float F-money" name="price" value="', 
                $line = 33, $out += $escape(otherInCome.price), $out += '" maxlength="11" ', $line = 34, 
                1 == otherInCome.isConfirmAccount && ($out += 'readOnly="readOnly"', $line = 34), 
                $out += ' /></td> <td><input style="width:90px;" type="text" class="F-float F-count" name="count" value="', 
                $line = 37, $out += $escape(otherInCome.count), $out += '" old="', $line = 37, $out += $escape(otherInCome.count), 
                $out += '" maxlength="11" ', $line = 38, 1 == otherInCome.isConfirmAccount && ($out += 'readOnly="readOnly"', 
                $line = 38), $out += '/></td> <td><span class="needPayMoney F-float F-money">', 
                $line = 40, $out += $escape(otherInCome.needPayMoney), $out += '</span><input type="hidden" name="needPayMoney" value="', 
                $line = 40, $out += $escape(otherInCome.needPayMoney), $out += '" ', $line = 41, 
                1 == otherInCome.isConfirmAccount && ($out += 'readOnly="readOnly"', $line = 41), 
                $out += "/></span></td> <td>", $line = 43, null != otherInCome.billImage && "" != otherInCome.billImage ? ($out += ' <a href="javascript:void(0);" url="', 
                $line = 44, $out += $escape(otherInCome.billImage), $out += '" class="btn-view">查看</a> ', 
                $line = 45) : ($out += ' <span style="color:#bbb;">查看</span> ', $line = 47), $out += " </td> <td>", 
                $line = 50, 1 == editStatus ? ($out += '<input name="billRemark" value="', $line = 50, 
                $out += $escape(otherInCome.billRemark), $out += '" maxlength="1000" ', $line = 50, 
                1 == otherInCome.isConfirmAccount && ($out += 'readOnly="readOnly"', $line = 50), 
                $out += "/>", $line = 50) : ($line = 50, $out += $escape(otherInCome.billRemark), 
                $line = 50), $out += "</td> ", $line = 52, 2 == tripPlan.billStatus && ($out += "<td>", 
                $line = 52, 0 == otherInCome.isConfirmAccount ? ($out += "未对账", $line = 52) : ($out += "已对账", 
                $line = 52), $out += "</td>", $line = 52), $out += " </tr> ", $line = 54), $out += " ", 
                $line = 55;
            }), $out += " </tbody> </table> ", $line = 59, tripPlan.billStatus > -1 && ($out += ' <div style="width:60%;"><div> <label style="margin:5px 0px 0px 0px;">财务审核批注：</label> <input name="accountFinancialCheckComment" ', 
            $line = 62, 1 == tripPlan.billStatus && isFinance || ($out += 'readonly="readonly"', 
            $line = 62), $out += ' type="text" style="width:30%;" value="', $line = 62, remarkArrangeList.otherInRemark.length > 0 && ($line = 62, 
            $out += $escape(remarkArrangeList.otherInRemark[0].opCheckRemark), $line = 62), 
            $out += '" /> <label style="margin:5px 0px 0px 10px;">计调审核批注：</label> <input name="accountOPCheckComment" ', 
            $line = 65, 0 == tripPlan.billStatus && isOp || ($out += 'readonly="readonly"', 
            $line = 65), $out += ' type="text" style="width:30%;" value="', $line = 65, remarkArrangeList.otherInRemark.length > 0 && ($line = 65, 
            $out += $escape(remarkArrangeList.otherInRemark[0].financeCheckRemark), $line = 65), 
            $out += '" /> </div> </div>', $line = 67), new String($out);
        } catch (e) {
            throw {
                filename: $filename,
                name: "Render Error",
