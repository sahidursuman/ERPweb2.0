define(function(require, exports) {
    ...
    var count = {
        ...
        listSearch : function () {
			...
			count.getlistCount(0,"","","","","",endTime,startTime,"");
        },
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    if(result){
            			...
            			var financialCountUpdate = addTab(uKey, "单团审核", html);
		...
		bindShoppingSum : function(obj, formClass) {
			...
			$('.countUpdate #shopping').find('tr[shopId='+ shopId +']').find('input[name=consumeMoney]').each(function() {
				var t = $(this).val();
				sumMoney += parseFloat(t);
			});
			$('.'+formClass).find('#shopping').find('.sumMoney'+shopId).text(sumMoney);
		...
		changeTwoDecimalToString : function (floatvar) {
			return count.changeTwoDecimal(floatvar)+"";
		},
		viewLog : function(id) {
			$.ajax({
	    ...
	    viewImage : function(obj,WEB_IMG_URL_BIG,WEB_IMG_URL_SMALL) {
	    	...
	    	for(var i = 0; i < strs.length; i ++) {
	    		...
	    		if(s != null && s != "" && s.length > 0) {
		    		var image = {
		    				"WEB_IMG_URL_BIG":WEB_IMG_URL_SMALL+s,	//原图
		    				"WEB_IMG_URL_SMALL":WEB_IMG_URL_SMALL+s+"?imageView2/2/w/150",	//缩略图
		    		}
	    ...
	    downloadExcel : function(id) {
	    	window.open("http://www.baidu.com/?word="+id);
	    }
    }
