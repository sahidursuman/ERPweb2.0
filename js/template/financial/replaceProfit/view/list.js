/*TMODJS:{"debug":true,"version":237,"md5":"8420495a30342a136ec4b15114a9b9ec"}*/
    ...
    return require("../../../template")("financial/replaceProfit/view/list", function($data, $filename) {
        try {
            ...
            return $line = 1, $each(bookingOrderList, function(rs) {
                ...
                $line = 6, rs.group && ($line = 6, $out += $escape(rs.group.name), $line = 6), $out += "</td> <td> ", 
                $line = 8, "" == rs.financialHotel ? ($out += " - ", $line = 10) : ($out += " ", 
                $line = 11, $out += $escape(rs.financialHotel), $out += " ", $line = 12), $out += " </td> <td> ", 
                $line = 15, "" == rs.financialTicket ? ($out += " - ", $line = 17) : ($out += " ", 
                $line = 18, $out += $escape(rs.financialTicket), $out += " ", $line = 19), $out += " </td> <td> ", 
                $line = 22, "" == rs.financialScenic ? ($out += " - ", $line = 24) : ($out += " ", 
                $line = 25, $out += $escape(rs.financialScenic), $out += " ", $line = 26), $out += " </td> <td> ", 
                $line = 29, "" == rs.financialBus ? ($out += " - ", $line = 31) : ($out += " ", 
                $line = 32, $out += $escape(rs.financialBus), $out += " ", $line = 33), $out += ' </td> <td><a class="T-option T-needPayDetail F-float F-money">', 
